...
''')
    ...
    o.write('def '+createFuncName+'''(outfile=sys.stdout ,delim='    '):
...
def encodeUInt(o, name, type, numbits, required=None, arraylen=1,
               unavailable=None, verbose=False):
    ...
    '''
    if verbose:
        print '  encodeUInt:', name, type, numbits, 'Req:',required,
...
''')
    ...
    for field in msgET.xpath('field'):
        ...
        if not hasSubTag(field,'required'):
            o.write('        if None==options.' + varName +
                    ': parser.error("missing value for ' + varName + '")\n')
