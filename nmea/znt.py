...
NOTE: Violates the 80 character line limit in NMEA.

Returns "#" when ntp not ready.
...
class ZntLogger():
  ...
  def state_str(self):
      if not self.enabled:
        return 'update: NOT ENABLED'
      if not self.will_write():
        s = 'update: NO write...'
        if self.max_cnt is not None:
          s += '  remaining_cnt: %d' % (self.max_cnt - self.cnt_since_last)
        if self.max_sec is not None:
          s += '  remaining_sec: %.1f' % (self.max_sec - (time.time() - self.last_write) )
        return s
