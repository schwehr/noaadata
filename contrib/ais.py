...
class bitfield:
    ...
    # or "n/a".
    def __init__(self, name, width, dtype, oob, legend, validator=None, formatter=None):
...
cnb = (
    bitfield(
        "status", 4, "unsigned", 0, "Navigation Status", formatter=cnb_status_legends
    ),
    bitfield("turn", 8, "signed", -128, "Rate of Turn", formatter=cnb_rot_format),
    bitfield(
        "speed", 10, "unsigned", 1023, "Speed Over Ground", formatter=cnb_speed_format
    ),
    bitfield("accuracy", 1, "unsigned", None, "Position Accuracy"),
    bitfield("lon", 28, "signed", 0x6791AC0, "Longitude", formatter=cnb_latlon_format),
    ...
    bitfield("second", 6, "unsigned", None, "Time Stamp", formatter=cnb_second_format),
...
type9 = (
    bitfield("alt", 12, "unsigned", 4095, "Altitude", formatter=type9_alt_format),
    bitfield("speed", 10, "unsigned", 1023, "SOG", formatter=type9_speed_format),
    bitfield("accuracy", 1, "unsigned", None, "Position Accuracy"),
    bitfield("lon", 28, "signed", 0x6791AC0, "Longitude", formatter=cnb_latlon_format),
    bitfield("lat", 27, "signed", 0x3412140, "Latitude", formatter=cnb_latlon_format),
    bitfield(
    ...
    bitfield("second", 6, "unsigned", 60, "Time Stamp", formatter=cnb_second_format),
...
type17 = (
    spare(2),
    bitfield("lon", 18, "signed", 0x1A838, "Longitude", formatter=short_latlon_format),
    bitfield("lat", 17, "signed", 0xD548, "Latitude", formatter=short_latlon_format),
    spare(2),
...
type21 = (
    bitfield("type", 5, "unisigned", 0, "Aid type", formatter=aide_type_legends),
...
type22 = (
    ...
    bitfield(
        "ne_lon", 18, "unsigned", 0x1A838, "NE Longitude", formatter=short_latlon_format
    ),
    bitfield(
        "ne_lat", 17, "unsigned", 0xD548, "NE Latitude", formatter=short_latlon_format
    ),
    bitfield(
        "sw_lon", 18, "unsigned", 0x1A838, "SW Longitude", formatter=short_latlon_format
    ),
    bitfield(
        "sw_lat", 17, "unsigned", 0xD548, "SW Latitude", formatter=short_latlon_format
    ),
...
type24b = (
    ...
    dispatch("mmsi", [type24b1, type24b2], lambda m: 1 if str(m)[:2] == "98" else 0),
)
...
class BitVector:
    ...
    def ubits(self, start, width):
        ...
        fld = 0
        for i in range(
            start / BITS_PER_BYTE, (start + width + BITS_PER_BYTE - 1) / BITS_PER_BYTE
        ):
            fld <<= BITS_PER_BYTE
            fld |= self.bits[i]
    ...
    def __repr__(self):
        ...
        return (
            str(self.bitlen)
            + ":"
            + "".join("%02x" % d for d in self.bits[: (self.bitlen + 7) / 8])
        )
...
def parse_ais_messages(source, scaled=False):
    ...
    while True:
        ...
        if scaled:
            for i, (_name, value, _dtype, _legend, formatter) in enumerate(cooked):
                if formatter:
...
if __name__ == "__main__":
    ...
    for parsed in parse_ais_messages(sys.stdin, scaled):
        if json:
            print("{" + ",".join('"' + x[0] + '"=' + str(x[1]) for x in parsed) + "}")
        elif csv:
            print(",".join(str(x[1]) for x in parsed))
        else:
            for _name, value, _dtype, legend, _formatter in parsed:
