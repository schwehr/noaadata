...
def encode(params, validate=False):
    ...
    Fields in params:
      ...
      - lockid(aisstr6): Character identifier of the station
      ...
      - lockschedules(binary): up to 6 lock schedule reports
...
def decode(bv, validate=False):
    ...
    r['lockid']=aisstring.decode(bv[20:62])
...
def decodelockschedules(bv, validate=False):
    return bv[130:]
...
def printHtml(params, out=sys.stdout):
        ...
        out.write("\n")
        out.write("<tr>\n")
        out.write("<td>lockid</td>\n")
        out.write("<td>aisstr6</td>\n")
        if 'lockid' in params:
            out.write("    <td>"+str(params['lockid'])+"</td>\n")
            out.write("    <td>"+str(params['lockid'])+"</td>\n")
        out.write("</tr>\n")
        out.write("\n")
        out.write("<tr>\n")
        ...
        out.write("\n")
        out.write("<tr>\n")
        out.write("<td>lockschedules</td>\n")
        out.write("<td>binary</td>\n")
        if 'lockschedules' in params:
            out.write("    <td>"+str(params['lockschedules'])+"</td>\n")
            out.write("    <td>"+str(params['lockschedules'])+"</td>\n")
        out.write("</tr>\n")
        out.write("</table>\n")
...
def printKml(params, out=sys.stdout):
    ...
    out.write("    <Placemark>\n")
    out.write("        <name>"+str(params['lockid'])+"</name>\n")
...
def printFields(params, out=sys.stdout, format='std', fieldList=None, dbType='postgres'):
    ...
    if 'std'==format:
        ...
        if 'lockid' in params: out.write("    lockid:         "+str(params['lockid'])+"\n")
        ...
        if 'lockschedules' in params: out.write("    lockschedules:  "+str(params['lockschedules'])+"\n")
        elif 'csv'==format:
        ...
        out.write("<Document>\n")
        out.write("    <name>sls_lockorder</name>\n")
        printKml(params,out)
        out.write("</Document>\n")
        out.write("</kml>\n")
    else:
...
def sqlCreate(fields=None, extraFields=None, addCoastGuardFields=True, dbType='postgres'):
    ...
    if 'lockid' in fields: c.addVarChar('lockid',7)
    if dbType != 'postgres':
    ...
    if 'lockschedules' in fields: c.addBitVarying('lockschedules',1024)
...
lockid"""+delim+'42'+delim+"""Character identifier of the station
...
lockschedules"""+delim+'-1'+delim+"""up to 6 lock schedule reports
Total bits"""+delim+"""129"""+delim+"""Appears to take 1 slot with 39 pad bits to fill the last slot""")
...
class Testsls_lockorder(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        self.failUnlessEqual(r['lockid'],params['lockid'])
        ...
        self.failUnlessEqual(r['lockschedules'],params['lockschedules'])
...
def addMsgOptions(parser):
    ...
    parser.add_option('--time_min-field', dest='time_minField',metavar='uint',type='int'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--lockid-field', dest='lockidField',default='@@@@@@@',metavar='aisstr6',type='string'
        ,help='Field parameter value [default: %default]')
    ...
    parser.add_option('--pos_latitude-field', dest='pos_latitudeField',default=Decimal('91'),metavar='decimal',type='string'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--lockschedules-field', dest='lockschedulesField',metavar='binary',type='string'
        ,help='Field parameter value [default: %default]')
...
def main():
    ...
    if options.doEncode:
        ...
        if None==options.lockidField: parser.error("missing value for lockidField")
        ...
        if None==options.lockschedulesField: parser.error("missing value for lockschedulesField")
    msgDict = {
