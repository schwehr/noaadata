...
def printHtml(params, out=sys.stdout):
        out.write("<h3>sls_lockschedule</h3>\n")
...
def printFields(params, out=sys.stdout, format='std', fieldList=None, dbType='postgres'):
    '''Print a sls_lockschedule message to stdout.
...
def sqlCreate(fields=None, extraFields=None, addCoastGuardFields=True, dbType='postgres'):
    ...
    if fields is None:
        fields = fieldList
    c = sqlhelp.create('sls_lockschedule',dbType=dbType)
    c.addPrimaryKey()
...
def sqlInsert(params,extraParams=None,dbType='postgres'):
        ...
        i = sqlhelp.insert('sls_lockschedule',dbType=dbType)
...
vessel"""+delim+'90'+delim+"""Vessel Name
direction"""+delim+'1'+delim+"""Up bound/Down bound
ETA_month"""+delim+'4'+delim+"""Estimated time of arrival  month 1..12
ETA_day"""+delim+'5'+delim+"""Estimated time of arrival  day of the month 1..31
ETA_hour"""+delim+'5'+delim+"""Estimated time of arrival  UTC hours 0..23
ETA_min"""+delim+'6'+delim+"""Estimated time of arrival  minutes
reserved"""+delim+'19'+delim+"""Reserved bits for future use
Total bits"""+delim+"""130"""+delim+"""Appears to take 1 slot with 38 pad bits to fill the last slot""")
...
class Testsls_lockschedule(unittest.TestCase):
    '''Use testvalue tag text from each type to build test case the sls_lockschedule message'''
...
def addMsgOptions(parser):
    parser.add_option('-d','--decode',dest='doDecode',default=False,action='store_true',
                help='decode a "sls_lockschedule" AIS message')
    parser.add_option('-e','--encode',dest='doEncode',default=False,action='store_true',
                help='encode a "sls_lockschedule" AIS message')
    parser.add_option('--vessel-field', dest='vesselField',default='@@@@@@@@@@@@@@@',metavar='aisstr6',type='string'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--direction-field', dest='directionField',metavar='bool',type='int'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--ETA_month-field', dest='ETA_monthField',metavar='uint',type='int'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--ETA_day-field', dest='ETA_dayField',metavar='uint',type='int'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--ETA_hour-field', dest='ETA_hourField',metavar='uint',type='int'
        ,help='Field parameter value [default: %default]')
    parser.add_option('--ETA_min-field', dest='ETA_minField',metavar='uint',type='int'
        ,help='Field parameter value [default: %default]')
...
def main():
    ...
    if options.doEncode:
        ...
        if None==options.vesselField: parser.error("missing value for vesselField")
        if None==options.directionField: parser.error("missing value for directionField")
        if None==options.ETA_monthField: parser.error("missing value for ETA_monthField")
        if None==options.ETA_dayField: parser.error("missing value for ETA_dayField")
        if None==options.ETA_hourField: parser.error("missing value for ETA_hourField")
        if None==options.ETA_minField: parser.error("missing value for ETA_minField")
    msgDict = {
