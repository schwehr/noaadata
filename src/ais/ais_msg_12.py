...
def printFields(
    ...
    """Print a asrm message to stdout.
...
def sqlCreate(
    ...
    if fields is None:
        fields = fieldList
    c = sqlhelp.create("asrm", dbType=dbType)
    c.addPrimaryKey()
...
def sqlInsert(params, extraParams=None, dbType="postgres"):
    ...
    i = sqlhelp.insert("asrm", dbType=dbType)
...
Spare & 1 & Must be 0\\\\ \\hline \\hline
Total bits & 72 & Appears to take 1 slot with 96 pad bits to fill the last slot \\\\ \\hline
\\end{tabular}
\\caption{AIS message number 12: Addressed safety related message.  FIX: need to handle the -1 string length at the end}
\\label{tab:asrm}
\\end{table}
""")
...
class Testasrm(unittest.TestCase):
...
def addMsgOptions(parser):
    parser.add_option(
        ...
        default=False,
        action="store_true",
        help='decode a "asrm" AIS message',
    )
    parser.add_option(
        "-e",
        "--encode",
        ...
        default=False,
        action="store_true",
        help='encode a "asrm" AIS message',
    )
    parser.add_option(
