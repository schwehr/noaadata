...
def encode(params, validate=False):
    ...
    if "to" in params:
        ...
        bvList.append(aisstring.encode("@@@@@@@@@@@@@@@@@@@@", 120))
    if "radius" in params:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(params["radius"]), 10))
    else:
    ...
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["tomonth"]), 4))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["tohour"]), 5))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["tomin"]), 6))
...
def printFields(
    ...
    if format == "std":
        ...
        if "reason" in params:
            out.write("    reason:           " + str(params["reason"]) + "\n")
        if "from" in params:
            out.write("    from:             " + str(params["from"]) + "\n")
        if "to" in params:
            out.write("    to:               " + str(params["to"]) + "\n")
        if "radius" in params:
            out.write("    radius:           " + str(params["radius"]) + "\n")
        if "unit" in params:
            out.write("    unit:             " + str(params["unit"]) + "\n")
        if "closingday" in params:
            out.write("    closingday:       " + str(params["closingday"]) + "\n")
        if "closingmonth" in params:
            out.write("    closingmonth:     " + str(params["closingmonth"]) + "\n")
        if "fromhour" in params:
            out.write("    fromhour:         " + str(params["fromhour"]) + "\n")
        if "frommin" in params:
            out.write("    frommin:          " + str(params["frommin"]) + "\n")
        if "today" in params:
            out.write("    today:            " + str(params["today"]) + "\n")
        if "tomonth" in params:
            out.write("    tomonth:          " + str(params["tomonth"]) + "\n")
        if "tohour" in params:
            out.write("    tohour:           " + str(params["tohour"]) + "\n")
        if "tomin" in params:
...
def sqlCreate(
    ...
    if "fid" in fields:
        c.addInt("fid")
    if "reason" in fields:
        c.addVarChar("reason", 20)
    if "from" in fields:
        c.addVarChar("from", 20)
    if "to" in fields:
        c.addVarChar("to", 20)
    if "radius" in fields:
        c.addInt("radius")
    if "unit" in fields:
        c.addInt("unit")
    if "closingday" in fields:
        c.addInt("closingday")
    if "closingmonth" in fields:
        c.addInt("closingmonth")
    if "fromhour" in fields:
        c.addInt("fromhour")
    if "frommin" in fields:
        c.addInt("frommin")
    if "today" in fields:
        c.addInt("today")
    if "tomonth" in fields:
        c.addInt("tomonth")
    if "tohour" in fields:
        c.addInt("tohour")
    if "tomin" in fields:
...
unit"""
        ...
        + "2"
        + delim
        + """Unit of extension value for range field
closingday"""
        + delim
        + "5"
        + delim
