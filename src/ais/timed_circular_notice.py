...
def encode(params, validate=False):
    ...
    if "timetoexpire" in params:
        bvList.append(
            binary.setBitVectorSize(BitVector.from_int(params["timetoexpire"]), 15)
        )
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(32767), 15))
    if "radius" in params:
        bvList.append(
            binary.setBitVectorSize(
                BitVector.from_int(int(Decimal(params["radius"]) * Decimal("0.1"))), 14
            )
        )
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(16383), 14))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["areatype"]), 8))
...
def printHtml(params, out=sys.stdout):
    ...
    if "timetoexpire" in params:
        ...
        if str(params["timetoexpire"]) in timetoexpireDecodeLut:
            out.write(
                "<td>" + timetoexpireDecodeLut[str(params["timetoexpire"])] + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
...
def sqlCreate(
    ...
    if "radius" in fields:
        c.addDecimal("radius", 6, 0)
    if "areatype" in fields:
        c.addInt("areatype")
...
fid"""
        ...
        + "6"
        + delim
        + """Functional IDentifier - 63
month"""
        + delim
        + "4"
        + delim
        + """Start time of most recent notice  UTC month
day"""
        + delim
        + "5"
        + delim
        + """Start time of most recent notice  UTC day of the month 1..31
hour"""
        + delim
        + "5"
        + delim
        + """Start time of most recent notice  UTC hours 0..23
min"""
        + delim
        + "6"
        + delim
        + """Start time of most recent notice  UTC minutes
longitude"""
        + delim
        + "28"
        + delim
        + """Center of the area/zone  East West location
latitude"""
        + delim
        + "27"
        + delim
        + """Center of the area/zone  North South location
timetoexpire"""
        + delim
        + "15"
        + delim
...
radius"""
        ...
        + "14"
        + delim
        + """Distance from center of detection zone (lat/lon above).  Distance in increments of 10 meters
areatype"""
        + delim
        + "8"
        + delim
        + """What does this circular area represent
Total bits"""
        + delim
        + """168"""
        + delim
