...
def encode(params, validate=False):
    ...
    bvList = []
    bvList.append(binary.setBitVectorSize(BitVector.from_int(10), 6))
...
def printFields(
    ...
    if format == "std":
        ...
        if "Spare1" in params:
            out.write("    Spare1:           " + str(params["Spare1"]) + "\n")
        if "DestID" in params:
            out.write("    DestID:           " + str(params["DestID"]) + "\n")
        if "Spare2" in params:
...
Total bits"""
        ...
        + """72"""
        + delim
        + """Appears to take 1 slot with 96 pad bits to fill the last slot"""
    )
...
class Testutcquery(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        assert r["Spare1"] == params["Spare1"]
        assert r["DestID"] == params["DestID"]
...
def main():
    ...
    if options.doEncode:
        ...
        if options.DestIDField is None:
            parser.error("missing value for DestIDField")
    msgDict = {
        "MessageID": "10",
