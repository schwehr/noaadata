...
def encode(params, validate=False):
    ...
    bvList = []
    bvList.append(binary.setBitVectorSize(BitVector.from_int(18), 6))
    ...
    if params["RAIM"]:
        ...
        bvList.append(FalseBV)
    bvList.append(
        binary.setBitVectorSize(BitVector.from_int(params["CommStateSelector"]), 1)
    )
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["CommState"]), 19))
...
def printHtml(params, out=sys.stdout):
    ...
    if "display_flag" in params:
        ...
        if str(params["display_flag"]) in display_flagDecodeLut:
            out.write(
                "<td>" + display_flagDecodeLut[str(params["display_flag"])] + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
    out.write("</tr>\n")
    ...
    if "CommStateSelector" in params:
        ...
        if str(params["CommStateSelector"]) in CommStateSelectorDecodeLut:
            out.write(
                "<td>"
                + CommStateSelectorDecodeLut[str(params["CommStateSelector"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
...
def printFields(
    ...
    if format == "std":
        ...
        if "Reserved1" in params:
            out.write("    Reserved1:          " + str(params["Reserved1"]) + "\n")
        if "SOG" in params:
        ...
        if "cs_unit" in params:
            out.write("    cs_unit:            " + str(params["cs_unit"]) + "\n")
        if "display_flag" in params:
            out.write("    display_flag:       " + str(params["display_flag"]) + "\n")
        if "dsc_flag" in params:
            out.write("    dsc_flag:           " + str(params["dsc_flag"]) + "\n")
        if "band_flag" in params:
            out.write("    band_flag:          " + str(params["band_flag"]) + "\n")
        if "msg22_flag" in params:
            out.write("    msg22_flag:         " + str(params["msg22_flag"]) + "\n")
        if "mode_flag" in params:
            out.write("    mode_flag:          " + str(params["mode_flag"]) + "\n")
        if "RAIM" in params:
            out.write("    RAIM:               " + str(params["RAIM"]) + "\n")
        if "CommStateSelector" in params:
            out.write(
                "    CommStateSelector:  " + str(params["CommStateSelector"]) + "\n"
            )
        if "CommState" in params:
...
def sqlCreate(
    ...
    if "Spare" in fields:
        c.addInt("Spare")
    if "cs_unit" in fields:
        c.addBool("cs_unit")
    if "display_flag" in fields:
        c.addBool("display_flag")
    if "dsc_flag" in fields:
        c.addBool("dsc_flag")
    if "band_flag" in fields:
        c.addBool("band_flag")
    if "msg22_flag" in fields:
        c.addBool("msg22_flag")
    if "mode_flag" in fields:
        c.addBool("mode_flag")
    if "RAIM" in fields:
        c.addBool("RAIM")
    if "CommStateSelector" in fields:
        c.addInt("CommStateSelector")
    if "CommState" in fields:
        c.addInt("CommState")
...
cs_unit"""
        ...
        + "1"
        + delim
        + """Does this unit do Carrier Sense?
display_flag"""
        + delim
        + "1"
        + delim
        + """Does this class B unit have an integrated display?
dsc_flag"""
        + delim
        + "1"
        + delim
        + """Does it have dedicated or time-shared DSC radio function?
band_flag"""
        + delim
        + "1"
        + delim
        + """How flexible is the freq handling of the unit?
msg22_flag"""
        + delim
        + "1"
        + delim
        + """Can the unit handle msg 22?
mode_flag"""
        + delim
        + "1"
        + delim
        + """Assigned mode wrt to VDL slots
RAIM"""
        + delim
        + "1"
        + delim
...
CommState"""
        ...
        + "19"
        + delim
        + """Not decoded by this software yet
Total bits"""
        + delim
        + """168"""
        + delim
...
def addMsgOptions(parser):
    ...
    parser.add_option(
        "--CommStateSelector-field",
        dest="CommStateSelectorField",
        metavar="uint",
        type="int",
        help="Field parameter value [default: %default]",
    )
