...
@author: U{"""
    + __author__
    + """<http://xenon.stanford.edu/~schwehr/>}
@version: """
    + __version__
    + """
@copyright: 2006
...
def makeField(options, out=sys.stdout):
    ...
    if options.arrayLength:
        out.write('arraylength="' + str(o.arrayLength) + '" ')
    ...
    if True:
        ...
        if o.unavailable:
            tag(out, "unavailable", o.unavailable, indent)
        if o.scale:
            tag(out, "scale", o.scale, indent)

        if o.decimalPlaces:
            tag(out, "decimalplaces", o.decimalPlaces, indent)
        if o.units:
            tag(out, "units", o.units, indent)

        if o.required:
            tag(out, "required", o.required, indent)
...
def validate(options, notify=True):
    ...
    if o.unavailable and (o.type in numberTypes):
        try:
            ...
            ok = False
            if notify:
                print("unavailable must be an number")
    ...
    if o.scale:
        try:
            ...
            ok = False
            if notify:
                print("Scale must be an number")

    if o.lut and (o.type not in ["bool", "uint"]):  # ,'sint']):
        ok = False
        if notify:
            print("Lookup tables must be an unsigned integer")
...
if __name__ == "__main__":
    ...
    myparser.add_option(
        ...
        + (
            "".join(
                [
                    choice + ":" + str(typeBitsGuess[choice]) + " "
                    for choice in typeBitsGuess
                ]
            )[:-1]
        )
        + ")",
