...
def encode(params, validate=False):
    ...
    bvList = []
    bvList.append(binary.setBitVectorSize(BitVector.from_int(3), 6))
    ...
    if "NavigationStatus" in params:
        bvList.append(
            binary.setBitVectorSize(BitVector.from_int(params["NavigationStatus"]), 4)
        )
    else:
    ...
    if "SOG" in params:
        bvList.append(
            binary.setBitVectorSize(
                BitVector.from_int(int(Decimal(params["SOG"]) * Decimal("10"))), 10
            )
        )
    else:
    ...
    if "TrueHeading" in params:
        bvList.append(
            binary.setBitVectorSize(BitVector.from_int(params["TrueHeading"]), 9)
        )
    else:
...
def printHtml(params, out=sys.stdout):
    ...
    if "NavigationStatus" in params:
        ...
        if str(params["NavigationStatus"]) in NavigationStatusDecodeLut:
            out.write(
                "<td>"
                + NavigationStatusDecodeLut[str(params["NavigationStatus"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
...
def printFields(
    ...
    if format == "std":
        ...
        if "NavigationStatus" in params:
            out.write(
                "    NavigationStatus:   " + str(params["NavigationStatus"]) + "\n"
            )
        if "ROT" in params:
        ...
        if "RegionalReserved" in params:
            out.write(
                "    RegionalReserved:   " + str(params["RegionalReserved"]) + "\n"
            )
        if "Spare" in params:
...
def sqlCreate(
    ...
    if "UserID" in fields:
        c.addInt("UserID")
    if "NavigationStatus" in fields:
        c.addInt("NavigationStatus")
    if "ROT" in fields:
        c.addInt("ROT")
    if "SOG" in fields:
    ...
    if "TrueHeading" in fields:
        c.addInt("TrueHeading")
    if "TimeStamp" in fields:
        c.addInt("TimeStamp")
    if "RegionalReserved" in fields:
        c.addInt("RegionalReserved")
    if "Spare" in fields:
        c.addInt("Spare")
    if "RAIM" in fields:
        c.addBool("RAIM")
...
\\end{tabular}
\\caption{AIS message number 3: Scheduled position report}
\\label{tab:position}
\\end{table}
""")
...
ROT"""
        ...
        + "8"
        + delim
        + """Rate of turning.  Positive right; negative left.  BROKEN!
SOG"""
        + delim
        + "10"
        + delim
...
class Testposition(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        assert r["NavigationStatus"] == params["NavigationStatus"]
        assert r["ROT"] == params["ROT"]
        ...
        assert r["RegionalReserved"] == params["RegionalReserved"]
...
def main():
    ...
    if options.doEncode:
        ...
        if options.NavigationStatusField is None:
            parser.error("missing value for NavigationStatusField")
        if options.ROTField is None:
