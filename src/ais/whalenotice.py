...
def encode(params, validate=False):
    ...
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["sec"]), 6))
    ...
    if "timetoexpire" in params:
        bvList.append(
            binary.setBitVectorSize(BitVector.from_int(params["timetoexpire"]), 16)
        )
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 16))
    if "radius" in params:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(params["radius"]), 16))
    else:
...
def decodetimetoexpire(bv, validate=False):
    return int(bv[191:207])


def decoderadius(bv, validate=False):
    return int(bv[207:223])
...
def printFields(
    ...
    if format == "std":
        ...
        if "timetoexpire" in params:
            out.write("    timetoexpire:     " + str(params["timetoexpire"]) + "\n")
        if "radius" in params:
...
month & 4 & Time of most recent whale detection.  UTC month 1..12 \\\\ \\hline
...
sec & 6 & Time of most recent whale detection.  UTC seconds \\\\ \\hline
stationid & 42 & Identifier of the station that detected the whale.  (e.g. which buoy) \\\\ \\hline
...
timetoexpire & 16 & Seconds from the detection time until the notice expires \\\\ \\hline
radius & 16 & Distance from center of detection zone (lat/lon above)\\\\ \\hline \\hline
Total bits & 223 & Appears to take 2 slots with 201 pad bits to fill the last slot \\\\ \\hline
\\end{tabular}
...
month"""
        ...
        + "4"
        + delim
        + """Time of most recent whale detection.  UTC month 1..12
day"""
        + delim
        + "5"
        + delim
...
sec"""
        ...
        + "6"
        + delim
        + """Time of most recent whale detection.  UTC seconds
stationid"""
        + delim
        + "42"
        + delim
...
Total bits"""
        ...
        + """223"""
        + delim
        + """Appears to take 2 slots with 201 pad bits to fill the last slot"""
    )
...
def main():
    ...
    if options.doEncode:
        ...
        if options.secField is None:
            parser.error("missing value for secField")
