...
def encode(params, validate=False):
    ...
    if "Time_year" in params:
        bvList.append(
            binary.setBitVectorSize(BitVector.from_int(params["Time_year"]), 14)
        )
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 14))
    if "Time_month" in params:
        bvList.append(
            binary.setBitVectorSize(BitVector.from_int(params["Time_month"]), 4)
        )
    else:
    ...
    if "fixtype" in params:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(params["fixtype"]), 4))
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 4))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 10))
    if params["RAIM"]:
...
def decode(bv, validate=False):
    ...
    r["Position_longitude"] = Decimal(binary.signedIntFromBV(bv[79:107])) / Decimal(
        "600000"
    )
    r["Position_latitude"] = Decimal(binary.signedIntFromBV(bv[107:134])) / Decimal(
        "600000"
    )
...
def printFields(
    ...
    if format == "std":
        ...
        if "Time_year" in params:
            out.write("    Time_year:           " + str(params["Time_year"]) + "\n")
        if "Time_month" in params:
        ...
        if "Time_sec" in params:
            out.write("    Time_sec:            " + str(params["Time_sec"]) + "\n")
        if "PositionAccuracy" in params:
            out.write(
...
Time_year"""
        ...
        + "14"
        + delim
        + """Current time stamp  year 1-9999
Time_month"""
        + delim
        + "4"
        + delim
        + """Current time stamp  month 1..12
Time_day"""
        + delim
        + "5"
        + delim
        + """Current time stamp  day of the month 1..31
Time_hour"""
        + delim
        + "5"
        + delim
        + """Current time stamp  UTC hours 0..23
Time_min"""
        + delim
        + "6"
        + delim
        + """Current time stamp  minutes
Time_sec"""
        + delim
        + "6"
        + delim
...
Position_longitude"""
        ...
        + "28"
        + delim
        + """Location of base station  East West location
Position_latitude"""
        + delim
        + "27"
        + delim
        + """Location of base station  North South location
fixtype"""
        + delim
        + "4"
        + delim
...
class Testbsreport(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        assert r["Time_year"] == params["Time_year"]
        ...
        assert r["Time_sec"] == params["Time_sec"]
