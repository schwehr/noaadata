...
def encode(params, validate=False):
    ...
    bvList = []
    bvList.append(binary.setBitVectorSize(BitVector.from_int(23), 6))
    ...
    if "corner2_lat" in params:
        ...
        bvList.append(binary.bvFromSignedInt(108600, 17))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["StationType"]), 4))
    ...
    bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 22))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["TxRxMode"]), 4))
    bvList.append(
        binary.setBitVectorSize(BitVector.from_int(params["ReportingInterval"]), 4)
    )
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["QuietTime"]), 22))
...
def printHtml(params, out=sys.stdout):
    ...
    if "StationType" in params:
        ...
        if str(params["StationType"]) in StationTypeDecodeLut:
            out.write(
                "<td>" + StationTypeDecodeLut[str(params["StationType"])] + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
    out.write("</tr>\n")
    ...
    if "ReportingInterval" in params:
        ...
        if str(params["ReportingInterval"]) in ReportingIntervalDecodeLut:
            out.write(
                "<td>"
                + ReportingIntervalDecodeLut[str(params["ReportingInterval"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
...
def printFields(
    ...
    if format == "std":
        ...
        if "StationType" in params:
            out.write("    StationType:        " + str(params["StationType"]) + "\n")
        if "shipandcargo" in params:
        ...
        if "ReportingInterval" in params:
            out.write(
                "    ReportingInterval:  " + str(params["ReportingInterval"]) + "\n"
            )
        if "QuietTime" in params:
...
def sqlCreate(
    ...
    if "TxRxMode" in fields:
        c.addInt("TxRxMode")
    if "ReportingInterval" in fields:
        c.addInt("ReportingInterval")
    if "QuietTime" in fields:
        c.addInt("QuietTime")
    if "Spare3" in fields:
        c.addInt("Spare3")
...
shipandcargo"""
        ...
        + "8"
        + delim
        + """Type of ship and cargo type.  FIX: need to verify that I got this lookup table right.  Also need to handle entryranges in the lookup table
Spare2"""
        + delim
        + "22"
        + delim
...
ReportingInterval"""
        ...
        + "4"
        + delim
        + """Command the stations to report as given in table 17
QuietTime"""
        + delim
        + "22"
        + delim
        + """How long to be quiet
Spare3"""
        + delim
        + "6"
        + delim
...
Total bits"""
        ...
        + """180"""
        + delim
        + """Appears to take 2 slots with 244 pad bits to fill the last slot"""
    )
...
def addMsgOptions(parser):
    ...
    parser.add_option(
        "--ReportingInterval-field",
        dest="ReportingIntervalField",
        metavar="uint",
        type="int",
        help="Field parameter value [default: %default]",
    )
