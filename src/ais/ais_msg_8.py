...
def printFields(
    ...
    if "std" == format:
        ...
        if "fi" in params:
            out.write("    fi:               " + str(params["fi"]) + "\n")
        if "BinaryData" in params:
            out.write("    BinaryData:       " + str(params["BinaryData"]) + "\n")
        elif "csv" == format:
...
def sqlCreate(
    ...
    if "fi" in fields:
        c.addInt("fi")
    if "BinaryData" in fields:
        c.addBitVarying("BinaryData", 1024)
...
dac"""
        ...
        + "10"
        + delim
        + """Appid designated area code (country)
fi"""
        + delim
        + "6"
        + delim
        + """Appid functional identifier
BinaryData"""
        + delim
        + "-1"
        + delim
        + """Bits for a binary broadcast message
Total bits"""
        + delim
        + """55"""
        + delim
        + """Appears to take 1 slot with 113 pad bits to fill the last slot"""
    )
...
class Testbin_broadcast(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        self.assertEqual(r["fi"], params["fi"])
        self.assertEqual(r["BinaryData"], params["BinaryData"])
...
def main():
    ...
    if options.doEncode:
        ...
        if None == options.dacField:
            parser.error("missing value for dacField")
        if None == options.fiField:
            parser.error("missing value for fiField")
        if None == options.BinaryDataField:
            parser.error("missing value for BinaryDataField")
    msgDict = {
        "MessageID": "8",
