...
def printFields(
    ...
    if format == "std":
        ...
        if "lockid" in params:
            out.write("    lockid:         " + str(params["lockid"]) + "\n")
        if "pos_longitude" in params:
...
def sqlCreate(
    ...
    if "lockschedules" in fields:
        c.addBitVarying("lockschedules", 1024)
...
lockschedules"""
        ...
        + "-1"
        + delim
        + """up to 6 lock schedule reports
Total bits"""
        + delim
        + """129"""
        + delim
        + """Appears to take 1 slot with 39 pad bits to fill the last slot"""
    )
