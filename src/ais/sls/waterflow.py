...
def encode(params, validate=False):
    ...
    if "flow" in params:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(params["flow"]), 10))
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(16383), 10))
...
def decodeflow(bv, validate=False):
    return int(bv[111:121])
...
def printHtml(params, out=sys.stdout):
    ...
    out.write("\n")
    out.write("<tr>\n")
    out.write("<td>flow</td>\n")
    out.write("<td>uint</td>\n")
    if "flow" in params:
        out.write("    <td>" + str(params["flow"]) + "</td>\n")
        out.write("    <td>" + str(params["flow"]) + "</td>\n")
    out.write("<td>m^3/s</td>\n")
    out.write("</tr>\n")
    out.write("\n")
    out.write("<tr>\n")
...
def printFields(
    ...
    if format == "std":
        ...
        if "flow" in params:
            out.write("    flow:           " + str(params["flow"]) + "\n")
        if "reserved" in params:
...
def main():
    ...
    if options.doEncode:
        ...
        if options.flowField is None:
            parser.error("missing value for flowField")
    msgDict = {
