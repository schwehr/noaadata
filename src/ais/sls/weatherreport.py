...
def encode(params, validate=False):
    ...
    if "speed" in params:
        bvList.append(
            binary.setBitVectorSize(
                BitVector.from_int(int(Decimal(params["speed"]) * Decimal("10"))), 10
            )
        )
    else:
    ...
    if "gust" in params:
        bvList.append(
            binary.setBitVectorSize(
                BitVector.from_int(int(Decimal(params["gust"]) * Decimal("10"))), 10
            )
        )
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(1023), 10))
    if "direction" in params:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(params["direction"]), 9))
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(511), 9))
    if "atmpressure" in params:
        bvList.append(
            binary.setBitVectorSize(
                BitVector.from_int(int(Decimal(params["atmpressure"]) * Decimal("10"))),
                14,
            )
        )
    else:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(163830), 14))
    if "airtemp" in params:
        bvList.append(
            binary.bvFromSignedInt(int(Decimal(params["airtemp"]) * Decimal("10")), 10)
        )
    else:
    ...
    if "visibility" in params:
        bvList.append(
            binary.setBitVectorSize(
                BitVector.from_int(int(Decimal(params["visibility"]) * Decimal("10"))),
                8,
            )
        )
    else:
...
def printFields(
    ...
    if format == "std":
        ...
        if "speed" in params:
            out.write("    speed:          " + str(params["speed"]) + "\n")
        if "gust" in params:
            out.write("    gust:           " + str(params["gust"]) + "\n")
        if "direction" in params:
            out.write("    direction:      " + str(params["direction"]) + "\n")
        if "atmpressure" in params:
            out.write("    atmpressure:    " + str(params["atmpressure"]) + "\n")
        if "airtemp" in params:
            out.write("    airtemp:        " + str(params["airtemp"]) + "\n")
        if "dewpoint" in params:
            out.write("    dewpoint:       " + str(params["dewpoint"]) + "\n")
        if "visibility" in params:
            out.write("    visibility:     " + str(params["visibility"]) + "\n")
        if "watertemp" in params:
...
def sqlCreate(
    ...
    if "speed" in fields:
        c.addDecimal("speed", 4, 1)
    if "gust" in fields:
        c.addDecimal("gust", 4, 1)
    if "direction" in fields:
        c.addInt("direction")
    if "atmpressure" in fields:
        c.addDecimal("atmpressure", 5, 1)
    if "airtemp" in fields:
        c.addDecimal("airtemp", 4, 1)
    if "dewpoint" in fields:
        c.addDecimal("dewpoint", 4, 1)
    if "visibility" in fields:
        c.addDecimal("visibility", 3, 1)
...
stationid"""
        ...
        + "42"
        + delim
        + """Character identifier of the station
pos_longitude"""
        + delim
        + "25"
        + delim
...
class Testsls_weatherreport(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        self.assertAlmostEqual(r["speed"], params["speed"], 1)
        self.assertAlmostEqual(r["gust"], params["gust"], 1)
        assert r["direction"] == params["direction"]
        self.assertAlmostEqual(r["atmpressure"], params["atmpressure"], 1)
        self.assertAlmostEqual(r["airtemp"], params["airtemp"], 1)
        self.assertAlmostEqual(r["dewpoint"], params["dewpoint"], 1)
        self.assertAlmostEqual(r["visibility"], params["visibility"], 1)
...
def addMsgOptions(parser):
    ...
    parser.add_option(
        "--atmpressure-field",
        dest="atmpressureField",
        default=Decimal("16383"),
        metavar="udecimal",
        type="string",
        help="Field parameter value [default: %default]",
    )
...
def main():
    ...
    if options.doEncode:
        ...
        if options.speedField is None:
            parser.error("missing value for speedField")
        if options.gustField is None:
            parser.error("missing value for gustField")
        if options.directionField is None:
            parser.error("missing value for directionField")
        if options.atmpressureField is None:
            parser.error("missing value for atmpressureField")
        ...
        if options.visibilityField is None:
            parser.error("missing value for visibilityField")
