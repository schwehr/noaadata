...
def printFields(
    ...
    if "std" == format:
        ...
        if "MessageID1" in params:
            out.write("    MessageID1:       " + str(params["MessageID1"]) + "\n")
        if "SlotOffset" in params:
            out.write("    SlotOffset:       " + str(params["SlotOffset"]) + "\n")
        if "Spare" in params:
            out.write("    Spare:            " + str(params["Spare"]) + "\n")
        if "MessageID12" in params:
            out.write("    MessageID12:      " + str(params["MessageID12"]) + "\n")
        if "SlotOffset12" in params:
            out.write("    SlotOffset12:     " + str(params["SlotOffset12"]) + "\n")
        if "Spare2" in params:
            out.write("    Spare2:           " + str(params["Spare2"]) + "\n")
        if "DestID2" in params:
            out.write("    DestID2:          " + str(params["DestID2"]) + "\n")
        if "MessageID2" in params:
            out.write("    MessageID2:       " + str(params["MessageID2"]) + "\n")
        if "SlotOffset2" in params:
            out.write("    SlotOffset2:      " + str(params["SlotOffset2"]) + "\n")
        if "Spare3" in params:
...
def sqlCreate(
    ...
    if "DestID" in fields:
        c.addInt("DestID")
    if "MessageID1" in fields:
        c.addInt("MessageID1")
    if "SlotOffset" in fields:
        c.addInt("SlotOffset")
    if "Spare" in fields:
        c.addInt("Spare")
    if "MessageID12" in fields:
        c.addInt("MessageID12")
    if "SlotOffset12" in fields:
        c.addInt("SlotOffset12")
    if "Spare2" in fields:
        c.addInt("Spare2")
    if "DestID2" in fields:
        c.addInt("DestID2")
    if "MessageID2" in fields:
        c.addInt("MessageID2")
    if "SlotOffset2" in fields:
        c.addInt("SlotOffset2")
    if "Spare3" in fields:
        c.addInt("Spare3")
...
DestID"""
        ...
        + "30"
        + delim
        + """MMSI number of interrogated station
MessageID1"""
        + delim
        + "6"
        + delim
...
Total bits"""
        ...
        + """140"""
        + delim
        + """Appears to take 1 slot with 28 pad bits to fill the last slot"""
    )
...
def main():
    ...
    if options.doEncode:
        ...
        if None == options.MessageID1Field:
            parser.error("missing value for MessageID1Field")
        if None == options.SlotOffsetField:
            parser.error("missing value for SlotOffsetField")
        if None == options.MessageID12Field:
            parser.error("missing value for MessageID12Field")
        if None == options.SlotOffset12Field:
            parser.error("missing value for SlotOffset12Field")
        if None == options.DestID2Field:
            parser.error("missing value for DestID2Field")
        if None == options.MessageID2Field:
            parser.error("missing value for MessageID2Field")
        if None == options.SlotOffset2Field:
            parser.error("missing value for SlotOffset2Field")
    msgDict = {
        "MessageID": "15",
