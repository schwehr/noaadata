...
def encode(params, validate=False):
    ...
    bvList = []
    bvList.append(binary.setBitVectorSize(BitVector.from_int(17), 6))
    ...
    if "x" in params:
        bvList.append(
            binary.bvFromSignedInt(int(Decimal(params["x"]) * Decimal("600")), 18)
        )
    else:
        bvList.append(binary.bvFromSignedInt(108600, 18))
    if "y" in params:
        bvList.append(
            binary.bvFromSignedInt(int(Decimal(params["y"]) * Decimal("600")), 17)
        )
    else:
...
def printFields(
    ...
    if format == "std":
        ...
        if "x" in params:
            out.write("    x:                " + str(params["x"]) + "\n")
        if "y" in params:
            out.write("    y:                " + str(params["y"]) + "\n")
        if "Spare2" in params:
...
UserID"""
        ...
        + "30"
        + delim
        + """Base station identification number (MMSI)
Spare"""
        + delim
        + "2"
        + delim
...
Total bits"""
        ...
        + """79"""
        + delim
        + """Appears to take 1 slot with 89 pad bits to fill the last slot"""
    )
...
class Testgnss_correction(unittest.TestCase):
    ...
    def testEncodeDecode(self):
        ...
        self.assertAlmostEqual(r["x"], params["x"], 2)
        self.assertAlmostEqual(r["y"], params["y"], 2)
...
def main():
    ...
    if options.doEncode:
        ...
        if options.xField is None:
            parser.error("missing value for xField")
        if options.yField is None:
            parser.error("missing value for yField")
