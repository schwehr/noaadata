...
def printFields(
    ...
    if "std" == format:
        ...
        if "DestID1" in params:
            out.write("    DestID1:          " + str(params["DestID1"]) + "\n")
        if "SeqID1" in params:
            out.write("    SeqID1:           " + str(params["SeqID1"]) + "\n")
        if "DestID2" in params:
            out.write("    DestID2:          " + str(params["DestID2"]) + "\n")
        if "SeqID2" in params:
            out.write("    SeqID2:           " + str(params["SeqID2"]) + "\n")
        if "DestID3" in params:
            out.write("    DestID3:          " + str(params["DestID3"]) + "\n")
        if "SeqID3" in params:
            out.write("    SeqID3:           " + str(params["SeqID3"]) + "\n")
        if "DestID4" in params:
...
def sqlCreate(
    ...
    if "Spare" in fields:
        c.addInt("Spare")
    if "DestID1" in fields:
        c.addInt("DestID1")
    if "SeqID1" in fields:
        c.addInt("SeqID1")
    if "DestID2" in fields:
        c.addInt("DestID2")
    if "SeqID2" in fields:
        c.addInt("SeqID2")
    if "DestID3" in fields:
        c.addInt("DestID3")
    if "SeqID3" in fields:
        c.addInt("SeqID3")
    if "DestID4" in fields:
        c.addInt("DestID4")
    if "SeqID4" in fields:
        c.addInt("SeqID4")
...
UserID"""
        ...
        + "30"
        + delim
        + """Unique ship identification number (MMSI).  Also known as the Source ID
Spare"""
        + delim
        + "2"
        + delim
...
DestID1"""
        ...
        + "30"
        + delim
        + """MMSI destication to ACK
SeqID1"""
        + delim
        + "2"
        + delim
        + """Sequence ID of the message to be acknowledged
DestID2"""
        + delim
        + "30"
        + delim
