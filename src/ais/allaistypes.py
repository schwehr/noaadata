...
def encode(params, validate=False):
    ...
    bvList.append(
        binary.bvFromSignedInt(int(Decimal(params["aDecimal"]) * Decimal("10")), 16)
    )
...
def printFields(
    ...
    if format == "std":
        ...
        if "reqDecimal" in params:
            out.write("    reqDecimal:    " + str(params["reqDecimal"]) + "\n")
        if "unavail_uint" in params:
            out.write("    unavail_uint:  " + str(params["unavail_uint"]) + "\n")
        if "anUInt" in params:
            out.write("    anUInt:        " + str(params["anUInt"]) + "\n")
        if "anInt" in params:
            out.write("    anInt:         " + str(params["anInt"]) + "\n")
        if "aBool" in params:
            out.write("    aBool:         " + str(params["aBool"]) + "\n")
        if "aStr" in params:
            out.write("    aStr:          " + str(params["aStr"]) + "\n")
        if "anUDecimal" in params:
            out.write("    anUDecimal:    " + str(params["anUDecimal"]) + "\n")
        if "aDecimal" in params:
...
def sqlCreate(
    ...
    if "dac" in fields:
        c.addInt("dac")
    if "reqDecimal" in fields:
        c.addDecimal("reqDecimal", 3, 0)
    if "unavail_uint" in fields:
        c.addInt("unavail_uint")
    if "anUInt" in fields:
        c.addInt("anUInt")
    if "anInt" in fields:
        c.addInt("anInt")
    ...
    if "aStr" in fields:
        c.addVarChar("aStr", 5)
    if "anUDecimal" in fields:
        c.addDecimal("anUDecimal", 5, 1)
    if "aDecimal" in fields:
        c.addDecimal("aDecimal", 4, 0)
    if "aFloat" in fields:
        c.addReal("aFloat")
...
reqDecimal"""
        ...
        + "8"
        + delim
        + """required decimal value... FIX: scale or no?
unavail_uint"""
        + delim
        + "2"
        + delim
        + """Unavailable unsigned integer
anUInt"""
        + delim
        + "2"
        + delim
        + """NO unavailable unsigned integer
anInt"""
        + delim
        + "3"
        + delim
        + """NO unavailable signed integer
aBool"""
        + delim
        + "1"
        + delim
...
aStr"""
        ...
        + "30"
        + delim
        + """An ais string of 5 characters
anUDecimal"""
        + delim
        + "16"
        + delim
        + """An unsigned decimal.  Allow smaller numbers
aDecimal"""
        + delim
        + "16"
        + delim
...
Total bits"""
        ...
        + """126"""
        + delim
        + """Appears to take 1 slot with 42 pad bits to fill the last slot"""
    )
