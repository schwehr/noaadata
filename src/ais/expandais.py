...
def expandAis(inET, verbose=False):
    ...
    for inc in includeStructs:
        if verbose:
            print("inc:", inc.attrib["name"], "parent:", inc.getparent().tag)
        ...
        if len(inc.xpath("do_not_mangle_name")) > 0:
            if verbose:
                print("Not mangling", structName, baseName[:-1])
            baseName = ""
        ...
        inc.getparent().replace(
            inc, etree.Comment("Struct include of " + inc.attrib["name"] + " was here")
        )

        replacement = copy.deepcopy(structDef[0])

        postgis = (
            False  # Does this structure have an associated postgis data structure?
        )
        postgisName = None
        postgisType = None
        if "postgis_type" in replacement.attrib:
            ...
            if verbose:
                print("Found PostGIS datatype for structure:", postgisName, postgisType)
        ...
        for subfield in replacement.xpath("field"):
            ...
            if postgis:
                if verbose:
                    print("Annotating", subfield.attrib["name"], "with postgis info")
            ...
            if len(desc) == 1:
                ...
                print(
                    "WARNING: no description for subfield!!!!  Must have exactly one description field"
                )
...
def nukeStructs(inET, verbose=False):
    ...
    for struct in structs:
        if verbose:
            print("nuking struct:", struct.attrib["name"])
        struct.getparent().replace(
            struct, etree.Comment("Struct " + struct.attrib["name"] + " was here")
        )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-i", "--input-file", dest="inFilename", default=None, help="AIS to read from"
    )
    parser.add_option(
        "-o",
        "--output-file",
