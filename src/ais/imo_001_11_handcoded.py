...
def encode(params, validate=False):
    ...
    if "latitude" in params:
        bvList.append(
            binary.bvFromSignedInt(
                int(Decimal(params["latitude"]) * Decimal("60000")), 24
            )
        )
    else:
    ...
    if "longitude" in params:
        bvList.append(
            binary.bvFromSignedInt(
                int(Decimal(params["longitude"]) * Decimal("60000")), 25
            )
        )
    else:
    ...
    if "ice" in params:
        ...
        bvList.append(binary.setBitVectorSize(BitVector.from_int(3), 2))
    bvList.append(
        binary.setBitVectorSize(BitVector.from_int(0), 6)
    )  # FIX: add Ohmex style encoding
...
def decode(bv, validate=False):
    ...
    r["waterlevel"] += Decimal(str(binary.signedIntFromBV(bv[346:352]) / 100.0))
    return r
...
def printHtml(params, out=sys.stdout):
    ...
    if "airpressuretrend" in params:
        ...
        if str(params["airpressuretrend"]) in airpressuretrendDecodeLut:
            out.write(
                "<td>"
                + airpressuretrendDecodeLut[str(params["airpressuretrend"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
    ...
    if "waterleveltrend" in params:
        ...
        if str(params["waterleveltrend"]) in waterleveltrendDecodeLut:
            out.write(
                "<td>"
                + waterleveltrendDecodeLut[str(params["waterleveltrend"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
...
def printFields(
    ...
    if format == "std":
        ...
        if "hour" in params:
            out.write("        hour:              " + str(params["hour"]) + "\n")
        if "min" in params:
            out.write(" min:               " + str(params["min"]) + "\n")
        if "avewind" in params:
            out.write("     avewind:           " + str(params["avewind"]) + "\n")
        if "windgust" in params:
            out.write("    windgust:          " + str(params["windgust"]) + "\n")
        if "winddir" in params:
            out.write("     winddir:           " + str(params["winddir"]) + "\n")
        if "windgustdir" in params:
            out.write(" windgustdir:       " + str(params["windgustdir"]) + "\n")
        if "airtemp" in params:
            out.write("     airtemp:           " + str(params["airtemp"]) + "\n")
        if "relhumid" in params:
            out.write("    relhumid:          " + str(params["relhumid"]) + "\n")
        if "dewpoint" in params:
            out.write("    dewpoint:          " + str(params["dewpoint"]) + "\n")
        if "airpressure" in params:
            out.write(" airpressure:       " + str(params["airpressure"]) + "\n")
        if "airpressuretrend" in params:
            out.write(
                "    airpressuretrend:  " + str(params["airpressuretrend"]) + "\n"
            )
        if "horizvis" in params:
            out.write("    horizvis:          " + str(params["horizvis"]) + "\n")
        if "waterlevel" in params:
            out.write("  waterlevel:        " + str(params["waterlevel"]) + "\n")
        if "waterleveltrend" in params:
            out.write(
                "     waterleveltrend:   " + str(params["waterleveltrend"]) + "\n"
            )
        if "surfcurspeed" in params:
            out.write(
                "        surfcurspeed:      " + str(params["surfcurspeed"]) + "\n"
            )
        if "surfcurdir" in params:
            out.write("  surfcurdir:        " + str(params["surfcurdir"]) + "\n")
        if "curspeed2" in params:
            out.write("   curspeed2:         " + str(params["curspeed2"]) + "\n")
        if "curdir2" in params:
            out.write("     curdir2:           " + str(params["curdir2"]) + "\n")
        if "curlevel2" in params:
            out.write("   curlevel2:         " + str(params["curlevel2"]) + "\n")
        if "curspeed3" in params:
            out.write("   curspeed3:         " + str(params["curspeed3"]) + "\n")
        if "curdir3" in params:
            out.write("     curdir3:           " + str(params["curdir3"]) + "\n")
        if "curlevel3" in params:
            out.write("   curlevel3:         " + str(params["curlevel3"]) + "\n")
        if "sigwaveheight" in params:
            out.write(
                "       sigwaveheight:     " + str(params["sigwaveheight"]) + "\n"
            )
        if "waveperiod" in params:
            out.write("  waveperiod:        " + str(params["waveperiod"]) + "\n")
        if "wavedir" in params:
            out.write("     wavedir:           " + str(params["wavedir"]) + "\n")
        if "swellheight" in params:
            out.write(" swellheight:       " + str(params["swellheight"]) + "\n")
        if "swellperiod" in params:
            out.write(" swellperiod:       " + str(params["swellperiod"]) + "\n")
        if "swelldir" in params:
            out.write("    swelldir:          " + str(params["swelldir"]) + "\n")
        if "seastate" in params:
            out.write("    seastate:          " + str(params["seastate"]) + "\n")
        if "watertemp" in params:
            out.write("   watertemp:         " + str(params["watertemp"]) + "\n")
        if "preciptype" in params:
            out.write("  preciptype:        " + str(params["preciptype"]) + "\n")
        if "salinity" in params:
            out.write("    salinity:          " + str(params["salinity"]) + "\n")
        if "ice" in params:
...
def sqlCreate(
    ...
    if "min" in fields:
        c.addInt("min")
    if "avewind" in fields:
        c.addInt("avewind")
    if "windgust" in fields:
        c.addInt("windgust")
    if "winddir" in fields:
        c.addInt("winddir")
    if "windgustdir" in fields:
        c.addInt("windgustdir")
    if "airtemp" in fields:
        c.addDecimal("airtemp", 4, 1)
    if "relhumid" in fields:
        c.addInt("relhumid")
    if "dewpoint" in fields:
        c.addDecimal("dewpoint", 4, 1)
    if "airpressure" in fields:
        c.addDecimal("airpressure", 3, 0)
    if "airpressuretrend" in fields:
        c.addInt("airpressuretrend")
    if "horizvis" in fields:
        c.addDecimal("horizvis", 3, 1)
    if "waterlevel" in fields:
        c.addDecimal("waterlevel", 3, 1)
    if "waterleveltrend" in fields:
        c.addInt("waterleveltrend")
    if "surfcurspeed" in fields:
        c.addDecimal("surfcurspeed", 3, 1)
    if "surfcurdir" in fields:
        c.addInt("surfcurdir")
    if "curspeed2" in fields:
        c.addDecimal("curspeed2", 3, 1)
    if "curdir2" in fields:
        c.addInt("curdir2")
    if "curlevel2" in fields:
        c.addInt("curlevel2")
    if "curspeed3" in fields:
        c.addDecimal("curspeed3", 3, 1)
    if "curdir3" in fields:
        c.addInt("curdir3")
    if "curlevel3" in fields:
        c.addInt("curlevel3")
    if "sigwaveheight" in fields:
        c.addDecimal("sigwaveheight", 3, 1)
    if "waveperiod" in fields:
        c.addInt("waveperiod")
    if "wavedir" in fields:
        c.addInt("wavedir")
    if "swellheight" in fields:
        c.addDecimal("swellheight", 3, 1)
    if "swellperiod" in fields:
        c.addInt("swellperiod")
    if "swelldir" in fields:
        c.addInt("swelldir")
    if "seastate" in fields:
        c.addInt("seastate")
    if "watertemp" in fields:
        c.addDecimal("watertemp", 4, 1)
    if "preciptype" in fields:
        c.addInt("preciptype")
    if "salinity" in fields:
        c.addDecimal("salinity", 3, 1)
    if "ice" in fields:
        c.addInt("ice")
    if "Spare2" in fields:
        c.addInt("Spare2")
...
Total bits"""
        ...
        + """352"""
        + delim
        + """Appears to take 2 slots with 72 pad bits to fill the last slot"""
    )
...
def addMsgOptions(parser):
    ...
    parser.add_option(
        "--airpressure-field",
        dest="airpressureField",
        default=Decimal("1311"),
        metavar="udecimal",
        type="string",
        help="Field parameter value [default: %default]",
    )
    parser.add_option(
        "--airpressuretrend-field",
        dest="airpressuretrendField",
        default=3,
        metavar="uint",
        type="int",
        help="Field parameter value [default: %default]",
    ...
    parser.add_option(
        "--waterleveltrend-field",
        dest="waterleveltrendField",
        default=3,
        metavar="uint",
        type="int",
        help="Field parameter value [default: %default]",
