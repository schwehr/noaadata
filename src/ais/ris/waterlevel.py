...
def encode(params, validate=False):
    ...
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id1_id"]), 11))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id1_sign"]), 1))
    bvList.append(
        binary.bvFromSignedInt(
            int(Decimal(params["id1_waterlevel"]) * Decimal("100")), 11
        )
    )
    bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 2))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id2_id"]), 11))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id2_sign"]), 1))
    bvList.append(
        binary.bvFromSignedInt(
            int(Decimal(params["id2_waterlevel"]) * Decimal("100")), 11
        )
    )
    bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 2))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id3_id"]), 11))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id3_sign"]), 1))
    bvList.append(
        binary.bvFromSignedInt(
            int(Decimal(params["id3_waterlevel"]) * Decimal("100")), 11
        )
    )
    bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 2))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id4_id"]), 11))
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["id4_sign"]), 1))
    bvList.append(
        binary.bvFromSignedInt(
            int(Decimal(params["id4_waterlevel"]) * Decimal("100")), 11
        )
    )
    bvList.append(binary.setBitVectorSize(BitVector.from_int(0), 2))
...
def printFields(
    ...
    if format == "std":
        ...
        if "country" in params:
            out.write("    country:             " + str(params["country"]) + "\n")
        if "id1_id" in params:
            out.write("    id1_id:              " + str(params["id1_id"]) + "\n")
        if "id1_sign" in params:
            out.write("    id1_sign:            " + str(params["id1_sign"]) + "\n")
        if "id1_waterlevel" in params:
            out.write(
                "    id1_waterlevel:      " + str(params["id1_waterlevel"]) + "\n"
            )
        if "id1_i_have_no_idea" in params:
            out.write(
                "    id1_i_have_no_idea:  " + str(params["id1_i_have_no_idea"]) + "\n"
            )
        if "id2_id" in params:
            out.write("    id2_id:              " + str(params["id2_id"]) + "\n")
        if "id2_sign" in params:
            out.write("    id2_sign:            " + str(params["id2_sign"]) + "\n")
        if "id2_waterlevel" in params:
            out.write(
                "    id2_waterlevel:      " + str(params["id2_waterlevel"]) + "\n"
            )
        if "id2_i_have_no_idea" in params:
            out.write(
                "    id2_i_have_no_idea:  " + str(params["id2_i_have_no_idea"]) + "\n"
            )
        if "id3_id" in params:
            out.write("    id3_id:              " + str(params["id3_id"]) + "\n")
        if "id3_sign" in params:
            out.write("    id3_sign:            " + str(params["id3_sign"]) + "\n")
        if "id3_waterlevel" in params:
            out.write(
                "    id3_waterlevel:      " + str(params["id3_waterlevel"]) + "\n"
            )
        if "id3_i_have_no_idea" in params:
            out.write(
                "    id3_i_have_no_idea:  " + str(params["id3_i_have_no_idea"]) + "\n"
            )
        if "id4_id" in params:
            out.write("    id4_id:              " + str(params["id4_id"]) + "\n")
        if "id4_sign" in params:
            out.write("    id4_sign:            " + str(params["id4_sign"]) + "\n")
        if "id4_waterlevel" in params:
            out.write(
                "    id4_waterlevel:      " + str(params["id4_waterlevel"]) + "\n"
            )
        if "id4_i_have_no_idea" in params:
            out.write(
...
def sqlCreate(
    ...
    if "country" in fields:
        c.addVarChar("country", 2)
    if "id1_id" in fields:
        c.addInt("id1_id")
    if "id1_sign" in fields:
        c.addInt("id1_sign")
    if "id1_waterlevel" in fields:
        c.addDecimal("id1_waterlevel", 4, 2)
    if "id1_i_have_no_idea" in fields:
        c.addInt("id1_i_have_no_idea")
    if "id2_id" in fields:
        c.addInt("id2_id")
    if "id2_sign" in fields:
        c.addInt("id2_sign")
    if "id2_waterlevel" in fields:
        c.addDecimal("id2_waterlevel", 4, 2)
    if "id2_i_have_no_idea" in fields:
        c.addInt("id2_i_have_no_idea")
    if "id3_id" in fields:
        c.addInt("id3_id")
    if "id3_sign" in fields:
        c.addInt("id3_sign")
    if "id3_waterlevel" in fields:
        c.addDecimal("id3_waterlevel", 4, 2)
    if "id3_i_have_no_idea" in fields:
        c.addInt("id3_i_have_no_idea")
    if "id4_id" in fields:
        c.addInt("id4_id")
    if "id4_sign" in fields:
        c.addInt("id4_sign")
    if "id4_waterlevel" in fields:
        c.addDecimal("id4_waterlevel", 4, 2)
    if "id4_i_have_no_idea" in fields:
        c.addInt("id4_i_have_no_idea")
...
id1_id"""
        ...
        + "11"
        + delim
        + """One tide gauge measurement.  Station ID defined by ERI for each country
id1_sign"""
        + delim
        + "1"
        + delim
        + """One tide gauge measurement.  sign of the number in the waterlevel
id1_waterlevel"""
        + delim
        + "11"
        + delim
        + """One tide gauge measurement.  Water level at the sensor
id1_i_have_no_idea"""
        + delim
        + "2"
        + delim
        + """One tide gauge measurement.  What are these bits for?  They do not seem to be specified
id2_id"""
        + delim
        + "11"
        + delim
...
def addMsgOptions(parser):
    ...
    parser.add_option(
        "--id1_waterlevel-field",
        dest="id1_waterlevelField",
        metavar="decimal",
        type="string",
        help="Field parameter value [default: %default]",
    )
    ...
    parser.add_option(
        "--id2_waterlevel-field",
        dest="id2_waterlevelField",
        metavar="decimal",
        type="string",
        help="Field parameter value [default: %default]",
    )
    ...
    parser.add_option(
        "--id3_waterlevel-field",
        dest="id3_waterlevelField",
        metavar="decimal",
        type="string",
        help="Field parameter value [default: %default]",
    )
    ...
    parser.add_option(
        "--id4_waterlevel-field",
        dest="id4_waterlevelField",
        metavar="decimal",
        type="string",
        help="Field parameter value [default: %default]",
    )
