...
def encode(params, validate=False):
    ...
    bvList = []
    bvList.append(binary.setBitVectorSize(BitVector.from_int(21), 6))
    ...
    if "type" in params:
        bvList.append(binary.setBitVectorSize(BitVector.from_int(params["type"]), 5))
    else:
    ...
    if params["OffPosition"]:
        bvList.append(TrueBV)
    else:
        bvList.append(FalseBV)
    bvList.append(binary.setBitVectorSize(BitVector.from_int(params["status"]), 8))
    if params["RAIM"]:
        bvList.append(TrueBV)
    else:
        bvList.append(FalseBV)
    if params["virtual_aton_flag"]:
        bvList.append(TrueBV)
    else:
        bvList.append(FalseBV)
    if params["assigned_mode_flag"]:
        bvList.append(TrueBV)
    else:
        bvList.append(FalseBV)
...
def printHtml(params, out=sys.stdout):
    ...
    if "OffPosition" in params:
        ...
        if str(params["OffPosition"]) in OffPositionDecodeLut:
            out.write(
                "<td>" + OffPositionDecodeLut[str(params["OffPosition"])] + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
    out.write("</tr>\n")
    ...
    if "virtual_aton_flag" in params:
        ...
        if str(params["virtual_aton_flag"]) in virtual_aton_flagDecodeLut:
            out.write(
                "<td>"
                + virtual_aton_flagDecodeLut[str(params["virtual_aton_flag"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
    ...
    if "assigned_mode_flag" in params:
        ...
        if str(params["assigned_mode_flag"]) in assigned_mode_flagDecodeLut:
            out.write(
                "<td>"
                + assigned_mode_flagDecodeLut[str(params["assigned_mode_flag"])]
                + "</td>"
            )
        else:
            out.write("<td><i>Missing LUT entry</i></td>")
...
def printFields(
    ...
    if format == "std":
        ...
        if "OffPosition" in params:
            out.write("    OffPosition:         " + str(params["OffPosition"]) + "\n")
        if "status" in params:
            out.write("    status:              " + str(params["status"]) + "\n")
        if "RAIM" in params:
            out.write("    RAIM:                " + str(params["RAIM"]) + "\n")
        if "virtual_aton_flag" in params:
            out.write(
                "    virtual_aton_flag:   " + str(params["virtual_aton_flag"]) + "\n"
            )
        if "assigned_mode_flag" in params:
            out.write(
                "    assigned_mode_flag:  " + str(params["assigned_mode_flag"]) + "\n"
            )
        if "spare" in params:
...
def sqlCreate(
    ...
    if "timestamp" in fields:
        c.addInt("timestamp")
    if "OffPosition" in fields:
        c.addBool("OffPosition")
    if "status" in fields:
        c.addInt("status")
    if "RAIM" in fields:
        c.addBool("RAIM")
    if "virtual_aton_flag" in fields:
        c.addBool("virtual_aton_flag")
    if "assigned_mode_flag" in fields:
        c.addBool("assigned_mode_flag")
    if "spare" in fields:
        c.addInt("spare")
    if "spare2" in fields:
        c.addInt("spare2")  # Just in case someone uses these bits for something
...
MessageID"""
        ...
        + """6"""
        + delim
        + """AIS message number.  Must be 21 aka 'F'
RepeatIndicator"""
        + delim
        + """2"""
        + delim
...
type"""
        ...
        + """5"""
        + delim
        + """IALA type of aid-to-navigation
name"""
        + delim
        + """120"""
        + delim
...
longitude"""
        ...
        + """28"""
        + delim
        + """Location of the AtoN  East West location
latitude"""
        + delim
        + """27"""
        + delim
        + """Location of the AtoN  North South location
dimA"""
        + delim
        + """9"""
        + delim
...
timestamp"""
        ...
        + """6"""
        + delim
        + """UTC second when report was generated
OffPosition"""
        + delim
        + """1"""
        + delim
        + """True when the AtoN is off station
status"""
        + delim
        + """8"""
        + delim
...
virtual_aton_flag"""
        ...
        + """1"""
        + delim
        + """Does the unit physically exist?
assigned_mode_flag"""
        + delim
        + """1"""
        + delim
...
Total bits"""
        ...
        + """272"""
        + delim
        + """Appears to take 2 slots with 152 pad bits to fill the last slot"""
    )
...
def addMsgOptions(parser):
    ...
    parser.add_option(
        "--virtual_aton_flag-field",
        dest="virtual_aton_flagField",
        metavar="bool",
        type="int",
        help="Field parameter value [default: %default]",
    )
    parser.add_option(
        "--assigned_mode_flag-field",
        dest="assigned_mode_flagField",
        metavar="bool",
        type="int",
        help="Field parameter value [default: %default]",
    )
