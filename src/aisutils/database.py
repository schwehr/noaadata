...
def stdCmdlineOptions(parser, dbType="postgres", verbose=False):
    ...
    if dbType in ("all", "postgres"):
        if verbose:
            sys.stderr.write("Adding postgres options\n")
        parser.add_option(
            "-d",
            "--database-name",
            dest="databaseName",
    ...
    if dbType in ("all", "sqlite"):
        if verbose:
            sys.stderr.write("Adding sqlite options\n")
        parser.add_option(
            "-f",
            "--database-file",
...
def createTables(
    ...
    for msgNum in ais.msgModByNumber:
        if excludeList is not None and msgNum in excludeList:
            continue
        if includeList is not None and msgNum not in includeList:
            continue
        ...
        if aisMod.dbTableName in tables:
            if verbose:
                sys.stderr.write(
                    str(msgNum)
                    + " ... skipping - already in the db -"
                    + str(aisMod.dbTableName)
                    + "\n"
                )
        else:
            if verbose:
    ...
    for msgNum in ais.msgModByNumber:
        ...
        if aisMod.dbTableName in tables:
            if verbose:
                print(
                    msgNum,
                    " ... skipping - already dropped from the db -",
                    aisMod.dbTableName,
                )
        else:
            if verbose:
...
def rebuild_track_lines(
    ...
    for vessel in vessels:
        ...
        if limitPoints is not None:
            query += " LIMIT " + str(limitPoints)
        query += ";"
        ...
        if v:
            sys.stderr.write(
                str(len(linePoints)) + " points used for vessel " + str(vessel) + "\n"
            )
        ...
        if len(track_keys) == 0:
            ...
            query = (
                "UPDATE track_lines SET name = %s, track = GeomFromText(%s,4326), update_timestamp = %s WHERE "
                + trackKey
                + " = %s"
            )
            key = track_keys[0][0]
            ...
            # How did this happen???
            sys.stderr.write(
                "ERROR: database corrupted ... too many track lines for "
                + str(vessel)
                + "\n"
            )
    ...
    if vesselsUpdated > 0:
        ...
        if v:
            sys.stderr.write(
                "Updated tracks ... " + str(vesselsUpdated) + " tracks updated\n"
            )
...
def rebuild_last_position(
    ...
    for vessel in vesselsClassA:
        ...
        if isinstance(startTime, datetime.date):
            sys.stderr.write(
                "WARNING: unexpected date object.  converting to a datetime\n"
            )
        ...
        if startTime is not None and cg_timestamp < startTime:
            cu.execute(
                "SELECT " + posKey + " FROM " + lastPosTable + " WHERE userid = %s;",
                (vessel,),
            )
            row = cu.fetchall()
            if len(row) > 0:
        ...
        cu.execute(
            "SELECT name,shipandcargo FROM shipdata WHERE userid="
            + str(vessel)
            + " LIMIT 1"
        )
        name = cu.fetchall()
        ...
        if len(lastpos_keys) == 0:
            ...
            query = (
                "INSERT INTO "
                + lastPosTable
                + " (userid,name,cog,sog,cg_timestamp,position) VALUES (%s,%s,%s,%s,%s,%s);"
            )
            try:
                # print query
                # if v:
            ...
            query = (
                "UPDATE "
                + lastPosTable
                + " SET name = %s, cog = %s, sog = %s, cg_timestamp=%s, position = %s WHERE "
                + posKey
                + " = %s"
            )
            ...
            # How did this happen???
            sys.stderr.write(
                "ERROR: database corrupted ... too many positions for "
                + str(vessel)
                + "\n"
            )
