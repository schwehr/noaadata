...
class Grid:
    ...
    def __init__(
        self, minx, miny, maxx, maxy, stepSize, gridType="occurrence", verbose=False
    ):
        """Prepare a grid.
        ...
        if gridType == "occurrence":
            ...
            self.grid = numpy.zeros(
                (self.xNumCells + 1, self.yNumCells + 1), dtype=float
            )
    ...
    def getCell(self, x, y, verbose=False):
        ...
        if verbose:
            print(x, y, "->", i, j, (x - self.miny), self.stepSize)
        return i, j
    ...
    def getLineCellsWithCrossings(self, x0, y0, x1, y1, verbose=False):
        ...
        if verbose:
            print("cellrange", startCell, endCell, x0, y0, "->", x1, y1)

        # just one cell
        if startCell == endCell:
            dist = distance(x0, y0, x1, y1)
            return ((startCell, 0.0, 1.0, dist),)
        ...
        if verbose:
            print("x_ycrossings", x_ycrossings)
        ...
        if verbose:
            ...
            print(
                "steps2",
                x_last_xcrossing - x_first_xcrossing,
                (x_last_xcrossing - x_first_xcrossing) / stepSize,
            )
        x_xcrossings = []
        for step in range(steps):
            x = step * stepSize + x_first_xcrossing
            if verbose:
                print("  X:", step, stepSize, x_first_xcrossing)
            x_xcrossings.append(x)
        if verbose:
            print("x_xcrossings", x_xcrossings)

        crossings = x_ycrossings + x_xcrossings
        if flippedX:
            if verbose:
                print("reverse sort crossings")
        ...
        for i in range(1, len(crossings)):
            ...
            if almostEqual(crossings[i], crossings[i - 1]):
                continue
        ...
        if not inBoundingBox(
            cells[0][0], cells[0][1], minCellX, minCellY, maxCellX, maxCellY
        ):
            if verbose:
                print("chomp front")
            raise ValueError(
    ...
    def getLineCells(self, x0, y0, x1, y1, verbose=False):
        ...
        if startCell[1] == endCell[1]:
            ...
            if flippedX:
                ...
                if verbose:
                    print("reversing based on x (horiz line)")
                cells.reverse()
            # print 'going to return',cells
            return cells

        if verbose:
            print("CASE not easy... must calculate")
        ...
        c = cells[0]
        if not inRange(c[0], minCellX, maxCellX) or not inRange(
            c[1], minCellY, maxCellY
        ):
            if verbose:
                print("chomp front")
            cells = cells[1:]
    ...
    def getMultiSegLineCellsWithCrossings(self, multiSegLine, verbose=False):
        ...
        for i in range(len(multiSegLine) - 1):
            newCells = self.getLineCellsWithCrossingsPts(
                multiSegLine[i], multiSegLine[i + 1]
            )
            cells += newCells
        return cells
