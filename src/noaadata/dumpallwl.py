...
Try to pull all the water level messages for a station over a long
time period.
"""
...
def noaawaterlevel2aisMsg8Nmea(stationID, mmsi, datum, wl, verbose=False, debug=False):
    ...
    """
    payloadBits = noaawaterlevel2aisBits(
        stationID, mmsi=mmsi, datum=datum, wl=wl, verbose=verbose, debug=debug
    )
...
def noaawaterlevel2aisBits(stationID, mmsi, datum, wl, verbose=False, debug=False):
    ...
    if params["o"] < 0:
        print(
            ...
            wl.O,
            params["o"],
            "FIX: what does this mean?  Forcing O to be positive",
        )
        params["o"] = -params["o"]

    if bool(wl.L):
        params["levelinferred"] = True
    else:
        params["levelinferred"] = False
    if bool(wl.F):
        params["flat_tolerance_exceeded"] = True
    else:
        params["flat_tolerance_exceeded"] = False
    if bool(wl.R):
        params["rate_tolerance_exceeded"] = True
    else:
