...
def dist_utm_km(p1, p2):
    return dist_utm_m(p1[0], p1[1], p2[0], p2[1]) / 1000.0
...
class Bbox:
    ...
    def is_inside(self, x, y):
        if self.bounds[0] is not None and self.bounds[0] > x:
            return False
        if self.bounds[2] is not None and self.bounds[2] < x:
            return False
...
def main():
    ...
    parser.add_option(
        ...
        default=60 * 10,
        type="int",
        help="Time in seconds before requiring a report [default: %default (sec) ]",
    )
    parser.add_option(
        "-d",
        "--delta-dist-m",
        default=200,
        type="int",
        help="Distant travel before requiring a report [default: %default (m) ]",
    )
    ...
    parser.add_option(
        ...
        default=None,
        type="int",
        help="Specify just one ship  [default: all ships]",
    )
    ...
    if options.mmsi is None:
        mmsi_list = [
            row["userid"]
            for row in cx.execute("SELECT distinct(userid) FROM position;")
        ]
    else:
    ...
    for mmsi in mmsi_list:
        decimate = Decimate(
            min_dist_m=options.delta_dist_m, min_time_s=options.delta_time_sec
        )
        if verbose:
            print(
                "Decimate options: %d m  and %d sec"
                % (options.delta_dist_m, options.delta_time_sec)
            )
        keep_cnt = 0
        toss_cnt = 0
        outside_cnt = 0
        ...
        for row_num, row in enumerate(
            cx.execute(
                "SELECT userid,longitude,latitude,cg_sec FROM position WHERE userid=%d AND latitude<90 ORDER by cg_sec;"
                % mmsi
            )
        ):
            if options.verbose and row_num % 10000 == 0:
                print(row_num)
            # print dict(row)
            ...
            if keep:
                ...
                d_with_tz = datetime.datetime(
                    d.year, d.month, d.day, d.hour, d.second, tzinfo=pytz.utc
                )
                ...
                csv.write(
                    "{mmsi},{x},{y},{d},{d_est}\n".format(
                        ...
                        x=x,
                        y=y,
                        d=d.strftime("%Y/%d/%m %H:%M:%S"),
                        d_est=d_est.strftime("%Y/%d/%m %H:%M:%S"),
                    )
                )
                # pass
            else:
