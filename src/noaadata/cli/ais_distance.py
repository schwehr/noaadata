...
def build_dist_database(database_filename, log_files, verbose=False):
    ...
    for filename in log_files:
        ...
        for line_num, line in enumerate(file(filename)):
            ...
            x = binary.signedIntFromBV(bits[61:89]) / 600000.0
            y = binary.signedIntFromBV(bits[89:116]) / 600000.0
            ...
            julian_day = int(
                datetime.datetime.utcfromtimestamp(int(match["timeStamp"])).strftime(
                    "%j"
                )
            )
            ...
            cu.execute(
                "INSERT INTO distance VALUES (:julian_day, :dist_km)",
                {
...
def main():
    ...
    if len(args) > 0:
        cx, counts = build_dist_database(
            options.database_filename, args, options.verbose
        )
        print(counts)
    else:
        ...
        distances = array(
            [
                ...
                for row in cx.execute(
                    "SELECT dist_km FROM distance WHERE julian_day=:julian_day AND dist_km<:max_dist_km",
