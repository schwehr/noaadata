...
def main():
    ...
    parser = OptionParser(
        usage="%prog [options]", option_class=magicdate.MagicDateOption
    )
    ...
    if verbose:
        logging.info(
            "time constraints %s %s", options.track_start, options.last_position_start
        )
    ...
    if options.updateTrackLines:
        ...
        if verbose:
            logging.info(
                "Updating track_lines (%s to %s)", startTime, datetime.datetime.utcnow()
            )

        aisutils.database.rebuild_track_lines(
            cx, limitPoints=options.limitPoints, startTime=startTime, verbose=verbose
        )
    ...
    if options.updateLastPosition:
        ...
        aisutils.database.rebuild_last_position(
            cx, startTime=startTime, verbose=verbose
        )
