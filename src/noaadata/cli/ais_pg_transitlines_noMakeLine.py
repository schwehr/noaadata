...
"""
    ...
    for rowNum, row in enumerate(cu.fetchall()):
        ...
        if count > 20000:
            if verbose:
                sys.stderr.write(" pre-committing before large data operation\n")
            cx.commit()
        ...
        sql_insert = (
            "INSERT INTO "
            + options.tableName
            + " (id,userid,track) VALUES (%s,%s,GeomFromText(%s,4326));"
        )
        ...
        if count > 20000:
            if verbose:
                sys.stderr.write("post-committing before large data operation\n")
            cx.commit()
