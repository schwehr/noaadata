...
def detectTransits(inFile, basename, options):
    ...
    if options.transitFile:
        summaryFile = file(
            basename + ".transits.summary.txt", "w"
        )  # Summary list of transits

    if options.gnuplot:
        gp = file(basename + ".gp", "w")
        os.chmod(basename + ".gp", 0o755)
    ...
    if options.excel:
        ...
        col = 0
        ws_summary.write(ws_summary_row, col, "MMSI")
        col += 1
        ws_summary.write(ws_summary_row, col, "Num transits")
        col += 1
        ws_summary.write(ws_summary_row, col, "Time in region (hours)")
        col += 1
        ws_summary.write(ws_summary_row, col, "AIS Position Count")
        col += 1
        ws_summary_row += 1
        ...
        ws_transits.write(
            ws_transits_row,
            0,
            "Vessel Transits based on AIS data produced by ais_transits.py",
        )
        ws_transits_row += 1
        ...
        ws_transits.write(ws_transits_row, col, "MMSI")
        col += 1
        ws_transits.write(ws_transits_row, col, "Transit")
        col += 1
        ws_transits.write(ws_transits_row, col, "Transit_ID")
        col += 1
        ws_transits.write(ws_transits_row, col, "Start (UTC sec)")
        col += 1
        ws_transits.write(ws_transits_row, col, "Start (UTC)")
        col += 1
        ws_transits.write(ws_transits_row, col, "Start (EDT/local)")
        col += 1
        ws_transits.write(ws_transits_row, col, "End (UTC sec)")
        col += 1
        ws_transits.write(ws_transits_row, col, "End (UTC)")
        col += 1
        ws_transits.write(ws_transits_row, col, "End (EDT/local)")
        col += 1
        ws_transits.write(ws_transits_row, col, "Transit Duration (hours)")
        col += 1
        ws_transits.write(ws_transits_row, col, "Transit Length (km)")
        col += 1
        ws_transits.write(ws_transits_row, col, "AIS Position Count")
        col += 1
        ws_transits_row += 1

    totalTransits = 0

    shipList = list(ships.keys())
    ...
    for ship in shipList:
        ...
        if options.separateShips:
            shipTransitFile = file(basename + "." + ship, "w")  # shipTransitFile
        ...
        print(ship)
        if transitsFile:
            transitsFile.write("# " + ship + "\n")
        if options.gmtMultiSeg:
            gmtMultiSegFile.write(
                ">\n"
            )  # This is the segment separator default character
        if options.separateShips:
            shipTransitFile.write("# " + ship + "\n")
        ...
        if options.excel:
            ...
            for tr in transits:
                ...
                ws_transits.write(ws_transits_row, col, str(ship))
                col += 1
                ws_transits.write(ws_transits_row, col, transitCount)
                col += 1
                ws_transits.write(ws_transits_row, col, ship + "_" + str(start))
                col += 1
                ws_transits.write(ws_transits_row, col, int(start))
                col += 1
                ws_transits.write(ws_transits_row, col, dt_start, dateTimeStyle)
                col += 1
                ws_transits.write(ws_transits_row, col, dt_start_local, dateTimeStyle)
                col += 1
                ws_transits.write(ws_transits_row, col, int(end))
                col += 1
                ws_transits.write(ws_transits_row, col, dt_end, dateTimeStyle)
                col += 1
                ws_transits.write(ws_transits_row, col, dt_end_local, dateTimeStyle)
                col += 1
                ws_transits.write(
                    ws_transits_row, col, (int(end) - int(start)) / 3600.0
                )
                col += 1
                ws_transits.write(ws_transits_row, col, samples)
                col += 1

                ws_transits_row += 1
            col = 0
            ws_summary.write(ws_summary_row, col, str(ship))
            col += 1
            ws_summary.write(ws_summary_row, col, len(transits))
            col += 1
            ws_summary.write(ws_summary_row, col, totalTime / 3600.0)
            col += 1
            ws_summary.write(ws_summary_row, col, totalSamples)
            col += 1
        ...
        if transitsFile:
            transitsFile.write("\n\n")
        if shipTransitFile:
            shipTransitFile.close()

        totalTransits += shipTransits

    if transitsFile:
        transitsFile.write("#total transits = " + str(totalTransits) + "\n")
