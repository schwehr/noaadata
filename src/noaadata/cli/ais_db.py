...
def main():
    ...
    if options.addPostgis:
        if options.dbType == "sqlite":
            sys.stderr.write(
                "WARNING: postgis not available for sqlite.  Ignoring request\n"
            )
        else:
            # TODO(schwehr): This no longer works.
            r = os.system(
                "psql -f /sw/share/doc/postgis83/contrib/postgis-1.5/postgis.sql -d "
                + options.databaseName
            )
            r = os.system(
                "psql -f /sw/share/doc/postgis83/contrib/postgis-1.5/spatial_ref_sys.sql -d "
                + options.databaseName
            )
    ...
    if options.createTrackLinesTable:
        ...
        sqlCreate += (
            " SELECT AddGeometryColumn('track_lines', 'track', 4326, 'LINESTRING', 2);"
        )
        if verbose:
