...
def suggestType(name, curType, printout=True):
    ...
    if printout:
        if newType:
            ...
            print(
                'Sorry!  No recommendation available for bad type "'
                + curType
                + '" for field "'
                + name
                + '"'
...
def generatePython(infile, outfile, prefixName=False, verbose=False):
    ...
    for msgET in aisMsgsET:
        ...
        buildDecodeParts(
            o, msgET, prefixName=prefixName
        )  # functions that only decode one field
...
def buildHelpers(o, msgET, verbose=False, prefixName=False):
    ...
    if prefixName:
        ...
        o.write("toPgFields = {\n")
    for field in msgET.xpath("field"):
        if "postgisName" not in field.attrib:
            continue
        name = field.attrib["name"]
...
def buildPrint(o, msgET, verbose=False, prefixName=False):
    ...
    for field in msgET.xpath("field"):
        ...
        if arraylen == 1 or fieldtype == "aisstr6":
            ...
            if not hasSubTag(field, "lookuptable"):
                ...
                if prefixName:
                    lutName = msgname + fieldname.capitalize() + "DecodeLut"
            ...
            sys.exit("FIX: handle arrays in the buildPrint func")

        if hasSubTag(field, "units"):
            o.write(
                '        out.write("<td>'
                + field.xpath("units")[0].text
                + '</td>\\n")\n'
            )
    ...
    if prefixName:
        printKmlName = msgName + "printKml"
    if not haveLocatableMessage(msgET):
        printKmlName = None
    else:
        ...
        o.write(
            '    out.write("        <name>"+str(params[\''
            + msgET.attrib["titlefield"]
            + '\'])+"</name>\\n")\n'
        )
    ...
    for field in msgET.xpath("field"):
        ...
        if arraylen == 1 or fieldtype == "aisstr6":
            o.write(
                ...
                + fieldname
                + "' in params: out.write(\"    "
                + padStrRight(fieldname + ":", maxFieldLen)
                + "  \"+str(params['"
                + fieldname
                + '\'])+"\\n")\n'
            )
...
def buildTextDef(o, msgET, verbose=False, prefixName=False):
    ...
    if prefixName:
        createFuncName = msgName + "textDefinitionTable"
...
"""
    ...
    for field in msgET.xpath("field"):
        ...
        lines.append(
            fieldName + '"""+delim+\'' + fieldBits + '\'+delim+"""' + fieldDescription
        )

    o.write("\n".join(lines))
    # 168 in the first slot and following slots get to use the whole 256??
    # FIX: is this right?
...
def buildLaTeX(o, msgET, verbose=False, prefixName=False):
    ...
    if prefixName:
        createFuncName = msgName + "LaTeXDefinitionTable"
...
""".replace("/", "\\\\")
    ...
    o.write(
        (
            """ // /hline
/end{tabular}
/caption{AIS message number """
            + msgET.attrib["aismsgnum"]
            + ": "
            + msgET.xpath("description")[0].text.replace("\n", " ")
            + """}
/label{tab:"""
            + msgET.attrib["name"]
            + """}
...
def buildSQL(o, msgET, verbose=False, prefixName=False):
    ...
    if prefixName:
        createFuncName = msgName + "SqlCreate"
...
""")
    ...
    o.write(
        "def "
        + createFuncName
        + """Str(outfile=sys.stdout, fields=None, extraFields=None
...
"""
    ...
    for field in msgET.xpath("field"):
        ...
        if fieldType in ("int", "uint"):
            o.write("Int ('" + fieldName + "')")
        elif fieldType in ("float"):
            o.write("Real('" + fieldName + "')")
        elif fieldType == "bool":
            o.write("Bool('" + fieldName + "')")
            ...
            if not hasSubTag(field, "scale"):
                print(
                    "ERROR: missing required <scale> for",
                    fieldName,
                    "of type",
                    fieldType,
                )
            ...
            print(
                "\n\n *** Unknown type in SQL code generation for field",
                fieldName + ":",
                fieldType,
                "\n\n",
            )
...
def buildLUT(o, msgET, verbose=False, prefixName=False):
    ...
    for field in msgET.xpath("field"):
        ...
        lutName = name
        if prefixName:
            lutName = msgname + name.capitalize()
...
def encodeFloat(
    ...
    if not unavailable:
        ...
        o.write("    if '" + name + "' in params:\n")
        o.write(
            "        bvList.append(binary.float2bitvec(params['" + name + "']" + "))\n"
        )
        o.write("    else:\n")
        o.write(
            "        bvList.append(binary.float2bitvec(" + str(unavailable) + "))\n"
        )
...
def encodeAisstr6(
    ...
    if required is not None:
        ...
        required = int(required)
        o.write(
            "    bvList.append(aisstring.encode('"
            + str(required)
            + "',"
            + totLen
            + "))\n"
    ...
    if not unavailable:
        o.write(
            "    bvList.append(aisstring.encode(params['"
            + name
            + "'],"
            + totLen
            + "))\n"
...
def encodeInt(
    ...
    if required is not None:
        ...
        required = int(required)
        o.write(
            "    bvList.append(binary.bvFromSignedInt("
            + str(required)
            + ","
            + str(numbits)
            + "))\n"
    ...
    if not unavailable:
        o.write(
            "    bvList.append(binary.bvFromSignedInt(params['"
            + name
            + "'],"
            + str(numbits)
            + "))\n"
...
def encodeDecimal(
    ...
    if not unavailable:
        o.write(
            "    bvList.append(binary.bvFromSignedInt(int(Decimal(params['"
            + name
            + "']"
            + offsetStr
            + ")*Decimal('"
...
def decodeInt(
    ...
    assert type == "int"
    if verbose:
        print(type, "decode", name, ": unvail=", unavailable, end=" ")
    ...
    if not decodeOnly:
        o.write("    " + dataDict + "['" + name + "']=")
    o.write(
        "binary.signedIntFromBV(" + bv + "[" + str(startindex) + ":" + str(end) + "])"
    )
    if not decodeOnly:
        o.write("\n")
    if verbose:
...
def decodeDecimal(
    ...
    if not decodeOnly:
        o.write("    " + dataDict + "['" + name + "']=")
    o.write(
        "Decimal(binary.signedIntFromBV("
        + bv
        + "["
        + str(startindex)
        + ":"
...
def decodeBinary(
    ...
    if int(numbits) != -1:
        o.write(str(end))  # -1 means go to the end of the message
    o.write("]")
    if not decodeOnly:
        o.write("\n")
...
def buildTestParamFunc(o, msgET, verbose=False, prefixName=False):
    ...
    if prefixName:
        o.write("def " + name + "TestParams():\n")
    else:
        o.write("def testParams():\n")
    o.write(
    ...
    for field in msgET.xpath("field"):
        ...
        if verbose:
            print("buildTestParamFunc ...", name, type)
        val = None
...
def buildUnitTest(o, msgET, verbose=False, prefixName=False):
    ...
    for field in msgET.xpath("field"):
        ...
        if type in ("bool", "uint", "int", "aisstr6", "binary"):
            ...
            places = "3"
            if hasSubTag(field, "decimalplaces"):
                places = field.xpath("decimalplaces")[0].text
            o.write(
                "        self.failUnlessAlmostEqual(r['"
                + name
                + "'],params['"
                + name
                + "'],"
...
def buildEncode(o, msgET, verbose=False, prefixName=False):
    ...
    name = msgET.attrib[
        "name"
    ]  # fix rename this variable to avoid hiding later on by field name

    print("Generating encoder ...", name)  # FIX: verbose?
    funcName = "encode"
    if prefixName:
        funcName = name + "Encode"
    ...
    o.write(
        "    '''Create a "
        + name
        + " binary message payload to pack into an AIS Msg "
        + name
        + ".\n\n"
    )
    o.write("    Fields in params:\n")
    ...
    for field in msgET.xpath("field"):
        ...
        if type == "bool":
            ...
            encodeUInt(o, name, type, numbits, required, arraylen, unavailable)
        elif type == "int":
            encodeInt(o, name, type, numbits, required, arraylen, unavailable)
        elif type == "float":
            encodeFloat(o, name, type, numbits, required, arraylen, unavailable)
        elif type == "aisstr6":
            encodeAisstr6(o, name, type, numbits, required, arraylen, unavailable)
        elif type == "decimal":
            scale = None
            if hasSubTag(field, "scale"):
                scale = field.xpath("scale")[0].text
            offset = None
            ...
            encodeBinary(o, name, type, numbits, required, arraylen, unavailable)
        else:
            print(
...
def buildDecode(o, msgET, verbose=False, prefixName=False):
    ...
    for field in msgET.xpath("field"):
        ...
        if hasSubTag(field, "optional"):
            ...
            if field.xpath("optional")[0].text is None:  # Found a NoReturn
                ...
                o.write(
                    "    if len(bv)<="
                    + str(startindex + pad)
                    + ": return r; # All fields below are optional\n"
                )
            elif text != "NoReturn":
        ...
        if type == "bool":
            startindex = decodeBool(
                o, name, type, startindex, numbits, required, arraylen, unavailable
            )
        elif type == "uint":
            startindex = decodeUInt(
                o, name, type, startindex, numbits, required, arraylen, unavailable
...
def buildOptParse(o, msgET, prefixName=False):
    ...
    if prefixName:
        funcName = msgName + "AddMsgOptions"
    o.write("\ndef " + funcName + "(parser):")
...
"""
    ...
    for field in msgET.xpath("field"):
        ...
        o.write(
            ...
            + fieldType
            + "',type='"
            + aisType2optParseType[fieldType]
            + "'"
        )
...
""")
    ...
    for field in msgET.xpath("field"):
        ...
        if not hasSubTag(field, "required"):
            o.write(
                "        if None==options."
                + varName
                + ': parser.error("missing value for '
                + varName
                + '")\n'
            )
    ...
    for field in msgET.xpath("field"):
        ...
        if hasSubTag(field, "required"):
            o.write(
                "        '" + name + "': '" + field.xpath("required")[0].text + "',\n"
            )
        else:
