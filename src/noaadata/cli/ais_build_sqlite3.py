...
def createTables(cx, verbose=False):
    ...
    if verbose:
        print(str(ais.ais_msg_4.sqlCreate(dbType="sqlite")))
...
def loadData(cx, datafile, verbose=False, uscg=True):
    ...
    for line in datafile:
        ...
        try:
            if msgNum == 1:
                ins = ais.ais_msg_1.sqlInsert(ais.ais_msg_1.decode(bv), dbType="sqlite")
            elif msgNum == 2:
                ins = ais.ais_msg_2.sqlInsert(ais.ais_msg_2.decode(bv), dbType="sqlite")
            elif msgNum == 3:
                ins = ais.ais_msg_3.sqlInsert(ais.ais_msg_3.decode(bv), dbType="sqlite")
            elif msgNum == 4:
                ins = ais.ais_msg_4.sqlInsert(ais.ais_msg_4.decode(bv), dbType="sqlite")
            elif msgNum == 5:
...
def main():
    ...
    if len(args) == 0:
        ...
        for filename in args:
            ...
            loadData(
                cx, file(filename, "r"), verbose=options.verbose, uscg=options.uscgTail
            )
