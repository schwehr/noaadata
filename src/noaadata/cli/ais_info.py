...
def get_sec_to_end_of_day(timestamp):
    ...
    d2 = datetime.datetime(
        year=d1.year, month=d1.month, day=d1.day, hour=23, minute=59, second=59
    )
    delta = d2 - d1
...
class Uptime:
    ...
    def set_end_time(self, timestamp):
        ...
        julian_yrday = int(
            datetime.datetime.utcfromtimestamp(self.max_sec).strftime("%Y%j")
        )

        if self.max_sec >= timestamp:
        ...
        if dt >= self.min_gap_sec:
            self.gap_counts[dt] = (
                1 if dt not in self.gap_counts else self.gap_counts[dt] + 1
            )
        self.gap_counts_raw[dt] = (
            1 if dt not in self.gap_counts_raw else self.gap_counts_raw[dt] + 1
        )

        if dt >= self.min_gap_sec:
            self.tot_by_julian_yrday[julian_yrday] = (
                dt_first
                if julian_yrday not in self.tot_by_julian_yrday
                else self.tot_by_julian_yrday[julian_yrday] + dt_first
            )
            self.tot_by_julian_yrday[julian_yrday + 1] = (
                dt_rest
                if julian_yrday + 1 not in self.tot_by_julian_yrday
                else self.tot_by_julian_yrday[julian_yrday + 1] + dt_rest
            )
    ...
    def add_time(self, timestamp):
        ...
        if self.verbose and self.max_sec is not None and self.max_sec > timestamp:
            ...
            print(
                ...
                timestamp,
                "    ",
                timestamp - self.max_sec,
                "\t\t",
                datetime.datetime.utcfromtimestamp(timestamp),
            )

        if self.min_sec is None:
            self.min_sec = timestamp
        self.max_sec = timestamp
    ...
    def up_time(self):
        ...
        keys = list(self.tot_by_julian_yrday.keys())
        keys.sort()
...
def distance_km_unit_sphere(lat1, long1, lat2, long2):
    return distance_m_unit_sphere(lat1, long1, lat2, long2) / 1000.0
...
class AisPositionStats:
    ...
    def add_message(self, ais_msg_dict, bits):
        ...
        if message_id in ("1", "2", "3"):
            ...
            x = lon = binary.signedIntFromBV(bits[61:89]) / 600000.0
            y = lat = binary.signedIntFromBV(bits[89:116]) / 600000.0
            ...
            if self.station_location is not None:
                dist = distance_km_unit_sphere(
                    x, y, self.station_location[0], self.station_location[1]
                )
                # print 'dist:', dist
    ...
    def add_file(self, filename):
        for _line_num, line in enumerate(file(filename)):
            ...
            if False:
                ...
                except AisErrorPositionTooFar as e:
                    print("ERROR: too far", str(e))
                    print("  Line:", line.rstrip())
                    continue
                except AisErrorBadNumBits as e:
...
def get_parser():
    ...
    parser.add_option(
        ...
        default=60 * 6,
        type="int",
        help="Suggest 21 seconds for a busy area with a basestation [default: %default]",
    )
    parser.add_option(
        "-s",
        "--start-time",
        type="magicdate",
        default=None,
        help="Force a start time (magicdate) [default: use first timestamp in file]",
    )
    parser.add_option(
        "-e",
        "--end-time",
        type="magicdate",
        default=None,
        help="Force an end  time (magicdate) [default: use last timestamp in file]",
    )
    parser.add_option(
        "--gap-file",
        default=None,
        help="base file name to store gap file [ default: %default ]",
    )

    parser.add_option(
        "--up-time-file",
        default=None,
        help="Where to write the uptime per day [default: file1.uptime]",
    )
    parser.add_option(
        "-v", "--verbose", default=False, action="store_true", help="Run in chatty mode"
    )
...
def main():
    ...
    info = AisStreamInfo(
        station_location=(-70.7165857810977, 42.1990684235934),
    ...
    if options.start_time is not None:
        ...
        if v:
            print("setting_start_time:", options.start_time, ts)
        info.up.set_start_time(ts)
        # print

    for file_num, filename in enumerate(args):
        if v:
            print("processing_file:", file_num, filename)
    ...
    if True:
        ...
        start_str = datetime.datetime.utcfromtimestamp(info.up.min_sec).strftime(
            "%Y-%m-%dT%H:%M"
        )
        end_str = datetime.datetime.utcfromtimestamp(info.up.max_sec).strftime(
            "%Y-%m-%dT%H:%M"
        )
        print(
            "time_range:",
