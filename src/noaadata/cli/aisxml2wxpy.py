...
""")
    ...
    for msgET in aisMsgsET.xpath("message"):
        ...
        if len(msgET.xpath("include-struct")) > 0:
            sys.exit(
                "ERROR: cannot handle xml that still has include-struct tags.\n  Please use expandais.py."
            )
...
""")
    for field in msgET.xpath("field"):
        if verbose:
            print("Does field", field.attrib["name"], "use choice ...", end=" ")
        if useChoice(field):
    ...
    if hasBoolField(msgET):
        o.write("\tBoolList=['False','True']\n")
    ...
    for field in msgET.xpath("field"):
        ...
        if hasSubTag(field, "required"):
            o.write(
                "\t\tself."
                + name
                + "Widget=wx.StaticText(self,label='"
            ...
            if "uint" == fieldType:
                if useChoice(field):
                    ...
                    o.write(
                        "wx.SpinCtrl(self,value=value,min=0,max="
                        + str(2**numBits - 1)
                        + ")\n"
                    )
            elif "int" == fieldType:
                ...
                if fieldType == "udecimal":
                    o.write(
                        "wx.Slider(self,-1,float(value),0,"
                        + str((2**numBits - 1) / scale)
                    )
                else:
                    # print name, numBits
                    # print 2**(numBits-1)
                    ...
                    o.write(
                        "wx.Slider(self,-1,float(value)," + str(start) + "," + str(end)
                    )
                o.write(",style=wx.SL_HORIZONTAL|wx.SL_AUTOTICKS | wx.SL_LABELS)\n")

            else:
                print(
                    "Please follow the GPS navigation system over the cliff",
                    name,
                    fieldType,
                )
                assert False
