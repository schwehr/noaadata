...
def handle_insert_update(cx, uscg_msg, msg_dict, aismsg):
    ...
    if msg_type in (1, 2, 3):
        ...
        if options.lon_min is not None and options.lon_min > x:
            return
        if options.lon_max is not None and options.lon_max < x:
            return
        if options.lat_min is not None and options.lat_min > y:
            return
        if options.lat_max is not None and options.lat_max < y:
            return
        ...
        cu.execute(
            "SELECT name,shipandcargo FROM shipdata WHERE userid=%s LIMIT 1;", (userid,)
        )
        row = cu.fetchall()
        if len(row) > 0:
            name = row[0][0].rstrip(" @")
        ...
        q = (
            "INSERT INTO last_position (userid,name,cog,sog,position,cg_r,navigationstatus, shipandcargo) VALUES (%s,%s,%s,%s,GeomFromText('POINT("
            + str(msg_dict["longitude"])
            + " "
            + str(msg_dict["latitude"])
            + ")',4326),%s,%s,%s);"
        )
    ...
    if msg_type == 18:
        ...
        cu.execute(
            "SELECT name FROM b_staticdata WHERE partnum=0 AND userid=%s LIMIT 1;",
            (userid,),
        )
        row = cu.fetchall()
        ...
        q = (
            "INSERT INTO last_position (userid,name,cog,sog,position,cg_r,shipandcargo) VALUES (%s,%s,%s,%s,GeomFromText('POINT("
            + str(msg_dict["longitude"])
            + " "
            + str(msg_dict["latitude"])
            + ")',4326),%s,%s);"
        ...
        cu.execute(
            q, (userid, name, msg_dict["COG"], msg_dict["SOG"], cg_r, shipandcargo)
        )
    ...
    if msg_type == 19:  # Class B extended report
        cu.execute(
            "DELETE FROM b_pos_and_shipdata WHERE userid=%s AND partnum=%s;",
            (userid, msg_dict["partnum"]),
        )
    ...
    if msg_type == 24:  # Class B static data report.  Either part A (0) or B (0)
        ...
        cu.execute(
            "DELETE FROM b_staticdata WHERE userid=%s AND partnum=%s;",
            (userid, msg_dict["partnum"]),
        )
...
class Nais2Postgis:
    ...
    def do_one_loop(self):
        ...
        if (self.db_last_commit_time is None) or (
            time.time() - self.db_last_commit_time > 30.0
            and self.db_uncommitted_count > 0
        ):
