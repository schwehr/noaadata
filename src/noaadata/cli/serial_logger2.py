...
def run(options):
    ...
    formatter = SerialLoggerFormatter(
        uscgFormat=options.uscgFormat, mark=options.mark, stationId=options.stationId
    )
    ...
    fh = MidnightRotatingFileHandler(
        options.log_prefix, epilogue=epilogue, prologue=prologue, rollover=rollover
    )
