...
def loadData(cx, datafile, verbose=False, uscg=True):
    ...
    for line in datafile:
        ...
        if True:
            if msgNum == 1:
                ins = ais.ais_msg_1.sqlInsert(
                    ais.ais_msg_1.decode(bv), dbType="postgres"
                )
            elif msgNum == 2:
                ins = ais.ais_msg_2.sqlInsert(
                    ais.ais_msg_2.decode(bv), dbType="postgres"
                )
            elif msgNum == 3:
                ins = ais.ais_msg_3.sqlInsert(
                    ais.ais_msg_3.decode(bv), dbType="postgres"
                )
            elif msgNum == 5:
                ...
                params["name"] = (
                    params["name"].replace('"', "").replace("\\", "").strip("@").strip()
                )
                params["callsign"] = (
                    params["callsign"]
                    .replace('"', "")
