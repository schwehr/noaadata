...
def assembleAisNmeaMessages(
    ...
    for line in infile:
        try:
            ...
            if len(fields) < 6:
                sys.stderr.write(
                    "ERROR line " + str(line_num) + ": not enough fields in line...\n"
                )
                sys.stderr.write(
                    "  Found " + str(len(fields)) + ", but need at least 6\n"
                )
            ...
            if totNumSentences == sentenceNum:
                ...
                for part in parts:
                    ...
                    if ts1 > ts2 + window or ts1 < ts2 - window:
                        sys.stderr.write(
                            "ERROR: timestamps not all the same for "
                            + str(timestamp)
                            + "\n"
                        )
                        sys.stderr.write("  ** " + line + "\n")
                ...
                if not isChecksumValid(out_str):
                    print(
                        "ERROR: Invalid checksum in constructed 1 liner:\t",
                        line,
                        end=" ",
                        file=sys.stderr,
                    )
    ...
    print("invalid checksums found...\t", end=" ", file=sys.stderr)
    print("%d\t(of %d)" % (invalid_checksums, line_num), file=sys.stderr)
