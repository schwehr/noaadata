...
class TrackDuplicates:
    ...
    def check_packet(self, time_sec, payload):
        ...
        for pkt in reversed(
            self.queue
        ):  # Search backwards since dups are likely close to the front, no?
...
def create_tables(cx, verbose=False):
    ...
    if verbose:
        print(str(ais.ais_msg_1_handcoded.sqlCreate(dbType="sqlite")))
    try:
    ...
    if verbose:
        print(str(ais.ais_msg_4_handcoded.sqlCreate(dbType="sqlite")))
    cu.execute(str(ais.ais_msg_4_handcoded.sqlCreate(dbType="sqlite")))

    if verbose:
        print(str(ais.ais_msg_5.sqlCreate(dbType="sqlite")))
    ...
    cu.execute(
        "ALTER TABLE position ADD est_sec_type INTEGER;"
    )  # Where did the est_sec come from?
...
def load_data(cx, datafile=sys.stdin, verbose=False, uscg=True):
    ...
    for line in datafile:
        ...
        if not nmea.checksum.isChecksumValid(line):
            ...
            print("   ", nmea.checksum.checksumStr(line), file=sys.stderr)
        ...
        try:
            ...
            print("ERROR: Unable to decode bits in line:\n\t", line, file=sys.stderr)
        ...
        try:
            if msg_num == 1:
                ...
                ins = ais.ais_msg_5.sqlInsert(ais.ais_msg_5.decode(bv), dbType="sqlite")
            elif msg_num == 18:
                ins = ais.ais_msg_18.sqlInsert(
                    ais.ais_msg_18.decode(bv), dbType="sqlite"
                )  # Class B position
            elif msg_num == 19:
                ins = ais.ais_msg_19.sqlInsert(
                    ais.ais_msg_19.decode(bv), dbType="sqlite"
                )  # Class B position
            else:
        ...
        if uscg:
            ...
            try:
                ...
                ins.add(
                    "cg_timestamp",
                    str(datetime.datetime.utcfromtimestamp(float(match["timeStamp"]))),
                )
                ins.add("cg_r", match["station"])
            except:
                print(match, file=sys.stderr)
                print("bad uscg sections", line, end=" ", file=sys.stderr)
                continue
            ...
            if match["time_of_arrival"] is not None:
                try:
                    ...
                    print(
                        "WARNING: corrupted time of arrival (T) in line.  T ignored\n\t",
                        line,
                        file=sys.stderr,
                    )
        ...
        if msg_num in (1, 2, 3, 4):
            pkt_id, dup_flag = track_dups.check_packet(
                cg_sec, fields[5]
            )  # Pass in the NMEA payload string of data
            if v:
