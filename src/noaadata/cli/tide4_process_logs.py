...
aanderraa_date_regex_str = (
    r"""Date/Time:\s+(?P<datetime>\d{1,2}.\d{1,2}.\d{1,2}\s+\d{1,2}:\d{1,2}:\d{1,2})"""
)
aanderraa_date_strptime = (
    "%y.%m.%d %H:%M:%S"  # !@#$!!!! this won't work directly either.  damn this is lame
)
...
aanderraa_reference_regex_str = (
    r"""(?P<entry_num>\d\d)\s+Reference\s+(?P<r1>\d+)\s+(?P<r2>\d+)"""
)
...
def main():
    ...
    o.write(
        "# Do not trust this data until you have verified it yourself.  Watch for timezone issue.\n"
    )
    ...
    for filename in glob.glob("*.C03"):
        ...
        basedate = datetime.datetime.strptime(
            "2009 %d %d:0:0" % (julianday, hour), "%Y %j %H:%M:%S"
        )
        ...
        for i in range(len(matches)):
            if matches[i][0] == "aand_date":
                ...
                startdate = datetime.datetime.strptime(
                    matches[i][1]["datetime"], aanderraa_date_strptime
                )
                break
            ...
            if len(cur_match) == 8:
                ...
                o.write(
                    output_format.format(
                        ...
                        date_aand=datetime.datetime.strptime(
                            m["aand_date"]["datetime"], aanderraa_date_strptime
                        ),
                        date_seabird=datetime.datetime.strptime(
                            m["seabird_ct"]["datetime"], seabird_ct_date_strptime
                        ),
                    )
                )
                cur_match = {}
