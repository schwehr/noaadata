...
def main():
    ...
    print("|USCG datetime| cg s | dt cg s | T | dT  | t | S | slot t |", end=" ")
    ...
    for line in file(sys.argv[1]):
        ...
        if match["body"][0] in ("1", "2", "3"):
            ...
            if "slot_number" not in msg:
                ...
                msg["slot_time"] = msg["slot_number"] / 2250.0 * 60
            if "commstate_utc_hour" not in msg:
                msg["commstate_utc_hour"] = msg["commstate_utc_min"] = "N/A"

            print(
                "{slot_number}|{slot_time}|{commstate_utc_hour}|{commstate_utc_min}|{TimeStamp}|{UserID}|".format(
                    **msg
                )
            )
        elif match["body"][0] == "4":
            ...
            print(
                "{slot_number}|{slot_time}|{Time_hour}|{Time_min}|{Time_sec}| b{UserID}|".format(
                    **msg
                )
            )
        else:
