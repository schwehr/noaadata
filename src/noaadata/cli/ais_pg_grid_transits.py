...
def get_parser():
    parser = OptionParser(
        usage="%prog [options] [transit_num] [transit_num] ...",
        version="%prog " + __version__,
    )
    parser.add_option(
        "-b", "--basename", default="tmp", help="Base file name for output"
    )
    ...
    parser.add_option(
        ...
        default=-70.7,
        type="float",
        help="Geographic lon range (-180..180) [default: %default]",
    )
    parser.add_option(
        "-X",
        "--x-max",
    ...
    parser.add_option(
        ...
        default=42.0,
        type="float",
        help="Geographic lat range (-90..90) [default: %default]",
    )
    parser.add_option(
        "-Y",
        "--y-max",
    ...
    parser.add_option(
        ...
        "--tracks-file",
        default=None,
        help="File containing space separated track id numbers [default: use args]",
    )
...
def main():
    ...
    g = grid.Grid(
        ll[0], ll[1], ur[0], ur[1], stepSize=options.step
    )  # , verbose=options.verbose)
    ...
    connectStr = (
        "dbname='{database_name}' user='{database_user}' host='{database_host}'".format(
            **options_dict
        )
    )
    if verbose:
