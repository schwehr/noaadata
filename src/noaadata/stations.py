...
def lonlatText2decimal(lonlatString):
    ...
    val = float(fields[0]) + float(fields[1]) / 60.0
...
class Station:
    ...
    def getLon(self):
        ...
        lon = float(lonFields[0]) + float(lonFields[1]) / 60.0
        if lonFields[2] == "W":
            lon = -lon
        return lon
    ...
    def getLat(self):
        ...
        lat = float(latFields[0]) + float(latFields[1]) / 60.0
        if latFields[2] == "S":
            lat = -lat
        return lat
    ...
    def getStationsInBBox(self, lowerleft, upperright):
        ...
        for station in self.stationsET.xpath("station"):
            ...
            try:
                ...
                if (
                    lon >= lowerleft[0]
                    and lon <= upperright[0]
                    and lat >= lowerleft[1]
                    and lat <= upperright[1]
                ):
