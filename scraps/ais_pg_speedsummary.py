...
if __name__ == "__main__":
    ...
    for category in categories:
        transitDist[category] = 0
    transitTime = {}
    for category in categories:
        transitTime[category] = datetime.timedelta(0)
    meanLength = {}
    lenStdErr = {}
    meanDraught = {}
    draughtStdErr = {}
    ...
    for category in categories:
        if verbose:
            sys.stderr.write("Category: " + category + "\n")

        minSpeeds = []
        maxSpeeds = []

        q = (
            "SELECT COUNT(id) FROM transit,(SELECT DISTINCT(userid) FROM summary2006 WHERE category='"
            + category
            + "') AS ships WHERE transit.userid=ships.userid;"
        )
        cu.execute(q)
        transits = cu.fetchone()[0]
        if verbose:
            sys.stderr.write("  Num transits: " + str(transits) + "\n")
        numberTransits[category] = transits

        cu.execute(
            "SELECT DISTINCT(userid) FROM summary2006 WHERE category='"
            + category
            + "';"
        )  # LIMIT 20;')
        ships = cu.fetchall()
        if verbose:
            sys.stderr.write("  Num ships: " + str(len(ships)) + "\n")
        ...
        for ship in ships:
            ...
            if shipCount % 20 == 0:
                print(" ", shipCount)
            q = (
                "SELECT t.id,startpos,endpos,length(Transform(track,32619)) FROM tpath"
                ",(SELECT id,startpos,endpos FROM transit WHERE userid='"
                + str(ship[0])
                + "') AS t "
                "WHERE tpath.id=t.id;"
            )
            ...
            for row in cu.fetchall():
                ...
                cu.execute(
                    "SELECT cg_timestamp FROM position WHERE key='"
                    + str(startpos)
                    + "';"
                )
                ts = cu.fetchone()[0]
                ...
                # else:
                transitDist[category] += lengthMeters / 1000.0
                # Transit time
                q = (
                    "SELECT cg_timestamp FROM position WHERE key IN ("
                    + str(startpos)
                    + ","
                    + str(endpos)
                    + ");"
                ...
                cu.execute(
                    "SELECT min(SOG),max(SOG) FROM position"
                    " WHERE userid="
                    + str(ship[0])
                    + " AND key>="
                    + str(startpos)
                ...
                if minSOG is None or maxSOG is None:
                    sys.stderr.write(
                        "Ship "
                        + str(ship[0])
                        + " has a transit with no motion.  Not counting towards speed\n"
                    )
                else:
        ...
        if verbose:
            sys.stderr.write(
                "  mean min/max speed: "
                + str(meanMinSpeed[category])
                + " ... "
                + str(meanMaxSpeed[category])
                + "\n"
            )

        if verbose:
            sys.stderr.write("  calculating avg length\n")
        cu.execute(
            "SELECT AVG(length) FROM summary2006 WHERE category='" + category + "';"
        )
        ...
        cu.execute(
            "SELECT length FROM summary2006 WHERE category='"
            + category
        ...
        cu.execute(
            "SELECT count(length) FROM summary2006 WHERE category='"
            + category
            + "' AND length IS NOT NULL;"
        )
        n = cu.fetchone()[0]
        ...
        cu.execute(
            "SELECT (SUM(firstdraft)+SUM(lastdraft))/(COUNT(firstdraft)+COUNT(lastdraft))"
            "FROM summary2006 WHERE category='"
            + category
        ...
        cu.execute(
            "SELECT count(firstdraft) FROM summary2006"
            " WHERE category='" + category + "' AND firstdraft IS NOT NULL;"
        )
        n1 = cu.fetchone()[0]
        cu.execute(
            "SELECT count(lastdraft) FROM summary2006"
        ...
        cu.execute(
            "SELECT firstdraft,lastdraft  FROM summary2006 WHERE category='"
            + category
            + "' "
            " AND firstdraft IS NOT NULL AND lastdraft IS NOT NULL"
        )
    ...
    ws.write(ws_row, 0, "Vessel Type Table")
    ws_row += 1
    ws.write(ws_row, 0, "Fishing vessels left out")
    ws_row += 1
    ws.write(ws_row, 0, "Class A only")
    ws_row += 1
    ws_row += 1

    col = 0
    ws.write(ws_row, col, "Vessel type")
    col += 1
    ws.write(ws_row, col, "# of vessels")
    col += 1
    ws.write(ws_row, col, "# of transits")
    col += 1
    ws.write(ws_row, col, "Tot transit distance (km)")
    col += 1
    ws.write(ws_row, col, "Tot transit time (days)")
    col += 1
    ws.write(ws_row, col, "Mean Vessel Length (m)")
    col += 1
    ws.write(ws_row, col, "StdErr")
    col += 1
    ws.write(ws_row, col, "Mean Vessel Draught (m)")
    col += 1
    ws.write(ws_row, col, "StdErr")
    col += 1
    ws.write(ws_row, col, "Mean Min Speed (kph)")
    col += 1
    ws.write(ws_row, col, "Mean Max Speed (kph)")
    col += 1

    for category in categories:
        ws_row += 1
        col = 0
        ws.write(ws_row, col, category)
        col += 1
        ws.write(ws_row, col, vesselCounts[category])
        col += 1
        # print(numberTransits[category], type(numberTransits[category]))
        ws.write(ws_row, col, int(numberTransits[category]))
        col += 1
        ws.write(ws_row, col, transitDist[category])
        col += 1
        dt = transitTime[category]
        days = dt.days + dt.seconds / (3600 * 24.0)
        print(" ", dt, "...", dt.days, dt.seconds, "->", days)
        ws.write(ws_row, col, days)
        col += 1
        ws.write(ws_row, col, meanLength[category])
        col += 1
        ws.write(ws_row, col, lenStdErr[category])
        col += 1
        ws.write(ws_row, col, meanDraught[category])
        col += 1
        ws.write(ws_row, col, draughtStdErr[category])
        col += 1
        ws.write(ws_row, col, knots2kph(meanMinSpeed[category]))
        col += 1
        ws.write(ws_row, col, knots2kph(meanMaxSpeed[category]))
        col += 1
