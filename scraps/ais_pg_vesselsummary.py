...
__version__ = ["$Revision:", "9918", "$"][1]
__revision__ = __version__  # For pylint
__date__ = ["$Date:", "2008-07-21", "14:12:23", "-0400", "(Mon,", "21", "Jul", "2008)", "$"][1]
__copyright__ = "2008"
...
def main():
    ...
    for vessel in vessels:
        ...
        max_draught = None
        cu.execute(
            "SELECT imonumber,callsign,name,shipandcargo,dimA,dimB,dimC,dimD,draught FROM shipdata WHERE userid=%s",
            (vessel,),
        )
        for row in cu.fetchall():
            imonumber, callsign, name, shipandcargo, dimA, dimB, dimC, dimD, draught = (
                row
            )
            ...
            if min_draught is None or draught < min_draught:
                min_draught = draught
            if max_draught is None or draught > max_draught:
                max_draught = draught
        ...
        for entry in shipdata:
            ...
            col = 0
            ws.write(ws_row, col, str(vessel))
            col += 1
            ws.write(ws_row, col, str(imonumber))
            col += 1
            ws.write(ws_row, col, callsign)
            col += 1
            ws.write(ws_row, col, name)
            col += 1
            ws.write(ws_row, col, shipandcargo)
            col += 1
            try:
                ...
                cargo_text = "unknown"
            ws.write(ws_row, col, cargo_text)
            col += 1

            ws.write(ws_row, col, length)
            col += 1
            ws.write(ws_row, col, width)
            col += 1
            ws.write(ws_row, col, min_draught)
            col += 1
            ws.write(ws_row, col, max_draught)
            col += 1
    ...
    workbook.save(
        "vessels-" + now.strftime("%Y-%m-%dT%H%M") + ".xls"
