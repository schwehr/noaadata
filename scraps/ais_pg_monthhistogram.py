...
if __name__ == "__main__":
    ...
    for category in categories:
        ...
        for ship in ships:
            if options.distance:
                q = (
                    "SELECT t.id,startpos,length(Transform(track,32619)) FROM tpath"
                    ",(SELECT id,startpos FROM transit WHERE userid='"
                    + str(ship[0])
                    + "') AS t "
                    "WHERE tpath.id=t.id;"
                )
                ...
                for row in cu.fetchall():
                    ...
                    if options.useNM:
                        monthCounts[ts.month] += (lengthMeters / 1000.0) * 0.539956803
                    else:
                        monthCounts[ts.month] += lengthMeters / 1000.0

                # sys.exit('early')
            else:  # do transit count
                cu.execute(
                    "SELECT p.cg_timestamp FROM position AS p, (SELECT startpos FROM transit WHERE userid='"
                    + str(ship[0])
                    + "') AS t WHERE key = startpos;"
                )
                for start in cu.fetchall():
                    monthCounts[start[0].month] += 1

        if verbose:
            sys.stderr.write("  " + str(monthCounts) + "\n")
...
"""
    ...
    if options.excel:
        ...
        ws.write(
            ws_row,
            0,
            "Ship transits for each month broken down by ship type.  Fishing left out",
        )
        ws_row += 1
        if options.distance:
            if options.useNM:
                ws.write(ws_row, 6, "Results in nautical miles (nm)")
                ws_row += 1
            else:
                ws.write(ws_row, 6, "Results in kilometers (km)")
                ws_row += 1
        else:
            ws.write(ws_row, 6, "Results in number of transits")
            ws_row += 1
        col = 0
        ws.write(ws_row, col, "Ship type")
        col += 1
        ws.write(ws_row, col, "Jan")
        col += 1
        ws.write(ws_row, col, "Feb")
        col += 1
        ws.write(ws_row, col, "Mar")
        col += 1
        ws.write(ws_row, col, "Apr")
        col += 1
        ws.write(ws_row, col, "May")
        col += 1
        ws.write(ws_row, col, "Jun")
        col += 1
        ws.write(ws_row, col, "Jul")
        col += 1
        ws.write(ws_row, col, "Aug")
        col += 1
        ws.write(ws_row, col, "Sep")
        col += 1
        ws.write(ws_row, col, "Oct")
        col += 1
        ws.write(ws_row, col, "Nov")
        col += 1
        ...
        for category in categories:
            ...
            for i in range(1, 13):
                ...
                ws.write(ws_row, col, int(catCounts[category][i]))
                col += 1
