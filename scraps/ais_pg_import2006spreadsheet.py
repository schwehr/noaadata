...
""")
    ...
    for filename in args:
        ...
        for line in file(filename):
            ...
            i = 0
            ins.add("userid", int(fields[i]))
            i += 1
            if len(fields[i]) > 0:
                ins.add("userid2", int(fields[i]))
            i += 1
            if len(fields[i]) > 0 and fields[i] != "NA":
                ins.add("imo_no", int(fields[i]))
            i += 1
            ins.add("numTransits", int(fields[i]))
            i += 1
            ...
            for mon in (
                ...
                if len(fields[i]) > 0:
                    ins.add(mon, int(fields[i]))
                else:
                    ins.add(mon, 0)
                i += 1

            ins.add("timeInRegion", float(fields[i]))
            i += 1
            ins.add("posCountAIS", int(fields[i]))
            i += 1
            name = fields[i].replace("'", " ")  # NOR'EASTER causes trouble
            ins.add("name", name)
            i += 1
            ins.add("shiptype", fields[i])
            i += 1
            ins.add("cargotype", fields[i])
            i += 1
            ins.add("catagory", fields[i])
            i += 1
            ins.add("vesseltype", fields[i])
            i += 1
            ins.add("vcargo", fields[i])
            i += 1
            if len(fields[i]) > 0:
                ins.add("length", float(fields[i]))
            i += 1
            if len(fields[i]) > 0:
                ins.add("beam", float(fields[i]))
            i += 1
            if len(fields[i]) > 0 and fields[i] != "NA":
                ins.add("firstDraft", float(fields[i]))
            i += 1
            if len(fields[i]) > 0 and fields[i] != "NA":
                ins.add("lastDraft", float(fields[i]))
            i += 1
            if len(fields[i]) > 0 and fields[i] != "NA":
                ins.add("grossTonnage", float(fields[i]))
            i += 1
            if len(fields[i]) > 0 and fields[i] != "NA":
                ins.add("yearBuilt", int(fields[i]))
            i += 1
            if len(fields[i]) > 0:
                ins.add("flag", fields[i])
            i += 1
            if len(fields[i]) > 0:
                ins.add("missingData", int(fields[i]))
            i += 1
            if len(fields[i]) > 0:
                ins.add("photoURL", fields[i])
            i += 1
            if len(fields[i]) > 0:
                ins.add("notes", fields[i].replace("'", " "))
            # i+=1
            print(str(ins))
            cu.execute(str(ins))
