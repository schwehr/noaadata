...
if __name__ == "__main__":
    ...
    if options.startDate is not None:
        ...
        dt = datetime.datetime(
            int(startFields[0]), int(startFields[1]), int(startFields[2])
        )

    if options.endDate is not None:
        endFields = options.endDate.split("-")
        dtend = datetime.datetime(
            int(endFields[0]), int(endFields[1]), int(endFields[2])
        )
    ...
    if options.startDate is None or options.endDate is None:
        ...
        if verbose:
            sys.stderr.write(
                "Date range found: " + str(minTS) + " TO " + str(maxTS) + "\n"
            )
        if options.startDate is None:
            dt = datetime.datetime(minTS.year, minTS.month, minTS.day)
        if options.endDate is None:
            dtend = datetime.datetime(maxTS.year, maxTS.month, maxTS.day)
        del minTS
        del maxTS

    if verbose:
        sys.stderr.write("Using date range: " + str(dt) + " to " + str(dtend) + "\n")
    ...
    while dt < dtend:
        ...
        if verbose and doy % 20 == 0:
            sys.stderr.write(str(doy) + " " + dateStr1 + "\n")
        dtnext = dt + oneDay
        dateStr2 = "%d-%02d-%02d" % (dtnext.year, dtnext.month, dtnext.day)
        q = (
            "SELECT count(distinct(userid)) FROM position"
            " WHERE cg_timestamp >= '"
            + dateStr1
            + "' AND cg_timestamp <= '"
            + dateStr2
    ...
    for day in range(7):
        o.write(
            str(day)
            + " "
            + str(dow[day] / float(dowCount[day]))
            + " "
            + dayNames[day]
            + "\n"
        )
