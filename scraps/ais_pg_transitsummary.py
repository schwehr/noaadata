...
def mark_utc(dt):
    ...
    return datetime.datetime(
        dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second, tzinfo=utc
    )
...
if __name__ == "__main__":
    ...
    ws.write(
        ws_row, 0, "Stats for each transit/skipped transits are single point issues"
    )
    ws_row += 1
    ...
    cu.execute(
        "SELECT cg_timestamp FROM position WHERE key=%s or key=%s;", (start, stop)
    )
    start = mark_utc(cu.fetchone()[0])
    stop = mark_utc(cu.fetchone()[0])

    col = 0
    ws.write(ws_row, col, "Time range (UTC):")
    col += 1
    ws_row += 1

    col = 0
    ws.write(ws_row, col, "Start:")
    col += 1
    ws.write(ws_row, col, start.strftime(datefmt))
    col += 1
    ws.write(ws_row, col, start.strftime(timefmt))
    col += 1
    ws.write(ws_row, col, "Stop:")
    col += 1
    ws.write(ws_row, col, stop.strftime(datefmt))
    col += 1
    ws.write(ws_row, col, stop.strftime(timefmt))
    col += 1
    ws_row += 1
    ...
    if options.timezone != pytz.timezone("UTC"):
        ...
        col = 0
        ws.write(ws_row, col, "Time range (" + options.timezone.zone + "):")
        col += 1
        ws_row += 1
    ...
    col = 0
    ws.write(ws_row, col, "Transit ID")
    col += 1
    ws.write(ws_row, col, "MMSI(UserID)")
    col += 1
    ws.write(ws_row, col, "StartKey")
    col += 1
    ws.write(ws_row, col, "EndKey")
    col += 1

    ws.write(ws_row, col, "Start Date (" + str(options.timezone) + ")")
    col += 1
    ws.write(ws_row, col, "Start Time (" + str(options.timezone) + ")")
    col += 1
    ws.write(ws_row, col, "End Date (" + str(options.timezone) + ")")
    col += 1
    ws.write(ws_row, col, "End Time (" + str(options.timezone) + ")")
    col += 1

    ws.write(ws_row, col, "Length(km)")
    col += 1
    ws.write(ws_row, col, "Min SOG(kts)")
    col += 1
    ws.write(ws_row, col, "Max SOG(kts)")
    col += 1
    ws.write(ws_row, col, "time > " + str(options.thresholdSpeed) + " kts (sec)")
    col += 1
    ws.write(ws_row, col, "total Time (sec)")
    col += 1
    ws.write(ws_row, col, "% time > " + str(options.thresholdSpeed) + " kts")
    col += 1
    ws.write(ws_row, col, "aisPos Count")
    col += 1
    ws.write(ws_row, col, "aisPos Used")
    col += 1
    ws.write(ws_row, col, "aisPos > " + str(options.thresholdSpeed) + " kts")
    col += 1
    ws_row += 1
    ...
    for rowcount, transit in enumerate(cu.fetchall()):
        ...
        if startkey == endkey:
            if verbose:
                sys.stderr.write(
                    "skipping transit id=" + str(t_id) + " ... only one point\n"
                )
            col = 0
            ws.write(ws_row, col, t_id)
            col += 1
            ws.write(ws_row, col, str(userid))
            col += 1
            ws.write(ws_row, col, startkey)
            col += 1
            ws.write(ws_row, col, endkey)
            col += 1
            ws.write(ws_row, col, "Start and end are the same position message")
            col += 1
            ws_row += 1
            continue
        # 32619 is UTM 19N WGS84
        cu.execute(
            "SELECT Length(Transform(track,32619))/1000. as length_km from tpath WHERE id=%s;",
            (t_id,),
        )
        try:
            ...
            length_km = -9999
        cu.execute(
            "SELECT MIN(sog),MAX(sog) FROM position WHERE key>=%s AND key<=%s AND userid=%s AND sog<%s;",
        ...
        if minSog is None or maxSog is None:
            ...
            ws.write(ws_row, col, endkey)
            col += 1
            ws.write(ws_row, col, "Min or max sog not available")
            col += 1
            ws_row += 1
            continue
        ...
        time_range_sql = (
            "SELECT MIN(cg_timestamp),MAX(cg_timestamp) FROM position WHERE userid=%d AND key>=%d AND key<=%d;"
        ...
        cu.execute(
            "SELECT MIN(cg_sec),MAX(cg_sec) FROM position WHERE userid=%d AND key>=%d AND key<=%d;"
            % (userid, startkey, endkey)
        )
        ...
        cu.execute(
            "SELECT sog,cg_sec FROM position WHERE key>=%s AND key<=%s AND userid=%s;",
        ...
        for row in cu.fetchall():
            ...
            if lastSEC == sec:
                if verbose:
                    sys.stderr.write(
                        "skipping transit point for "
                        + str(t_id)
                        + " ... adjacents points within the same second\n"
                    )
                    sys.stderr.write(
                        "  "
                        + str(pointCount)
            ...
            if lastSOG >= options.maxSpeed:
                sys.stderr.write(
                    "a - skipping transit point for "
                    + str(t_id)
                    + " ... 102.3 is an undefined speed for vessel "
                    + str(userid)
                    + "\n"
                )
                sys.stderr.write(
        ...
        cu.execute(
            "SELECT COUNT(*) FROM position WHERE key>=%s AND key<=%s AND userid=%s;",
            (startkey, endkey, userid),
        )
        ...
        cu.execute(
            "SELECT COUNT(*) FROM position WHERE key>=%s AND key<=%s AND userid=%s AND sog>=%s;",
        ...
        ws.write(ws_row, col, startTime.strftime(datefmt))
        col += 1
        ws.write(ws_row, col, startTime.strftime(timefmt))
        col += 1

        ws.write(ws_row, col, endTime.strftime(datefmt))
        col += 1
        ws.write(ws_row, col, endTime.strftime(timefmt))
        col += 1

        ws.write(ws_row, col, length_km)
        col += 1
        ws.write(ws_row, col, minSog)
        col += 1
        ws.write(ws_row, col, maxSog)
        col += 1

        ws.write(ws_row, col, totalSecAbove)
        col += 1
        ws.write(ws_row, col, totalT)
        col += 1

        ws.write(ws_row, col, percentOverThresh)
        col += 1
        ws.write(ws_row, col, aisPosCount)
        col += 1

        ws.write(ws_row, col, aisPosUsed)
        col += 1
        ws.write(ws_row, col, aisPosAboveThresh)
        col += 1
        ws_row += 1

    now = datetime.datetime.now()
    workbook.save(
        "results-" + now.strftime("%Y-%m-%d_%H%M") + ".xls"
