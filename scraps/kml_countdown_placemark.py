...
__version__ = ["$Revision:", "8283", "$"][1]
__date__ = ["$Date:", "2008-01-17", "16:20:22", "-0500", "(Thu,", "17", "Jan", "2008)", "$"][1]
__copyright__ = "2008"
...
def addStyle(
    out, color="ffffff", opacity=0.4, scale=1, indent="    ", styleName="style"
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--start-time", dest="start", type="magicdate", default=None, help="magicdate"
    )
    ...
    parser.add_option(
        ...
        default=150,
        type="float",
        help="Height above the surface" + " [default: %default m ]",
    )
    parser.add_option(
        "-x",
        "--lon",
        dest="x",
        default=-70.52368399114937,
        type="float",
    ...
    start = datetime.datetime(
        options.start.year, options.start.month, options.start.day
    ) + datetime.timedelta(seconds=60 * options.timeOffset)
...
""")
    ...
    for minutes in range(0, 24 * 60, options.timeStep):
        ...
        nowStr = "%4d-%02d-%02dT%02d:%02d:00Z" % (
            now.year,
            now.month,
            now.day,
            now.hour,
        ...
        nowStr_plus1 = "%4d-%02d-%02dT%02d:%02d:00Z" % (
            now.year,
            now.month,
            now.day,
            now.hour,
            now.minute + options.timeStep,
        )
...
<pre>  Restriction ends: <b>""")
            + str(start + datetime.timedelta(days=1))
            + """z</b>
  Whale detected:   """
            + str(start)
            + """z
